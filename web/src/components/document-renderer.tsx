...
import {
  AlertTriangle, ArrowUpRight, ChevronDown, ChevronRight, FileCheck2, Link2, ShieldCheck,
} from "lucide-react";
...
export function DocumentRenderer({ document, onOpenDependency }: { readonly document: HumanDocumentView; readonly onOpenDependency?: (id: string) => void }) {
  ...
  return <article className="document-view">
    ...
    {identity.length === 0 ? null : <DocumentIdentity sections={identity} />}
    <DocumentContent sections={sections.content} />
...
function MetadataValue({ field, value, documentType, date }: { readonly field: string; readonly value: unknown; readonly documentType: string; readonly date: (value: string | Date) => string }) {
  ...
  if (Array.isArray(value)) {
    ...
    return <ul className="metadata-value-list">{items.map((item, index) => <li key={index}><MetadataValue field="item" value={item} documentType={documentType} date={date} /></li>)}</ul>;
  }
  if (value !== null && typeof value === "object") {
    const entries = Object.entries(value as Readonly<Record<string, unknown>>);
    return <dl className="metadata-provenance">{entries.map(([key, item]) => <div key={key}><dt>{contractLabel(key)}</dt><dd><MetadataValue field={key} value={item} documentType={documentType} date={date} /></dd></div>)}</dl>;
  }
  return <span>—</span>;
}
