...
function GovernanceDialog({ projectId, resolving, onClose, onSaved }: { readonly projectId: string; readonly resolving?: GovernanceEventView; readonly onClose: () => void; readonly onSaved: () => void }) {
  ...
  return <Modal title={resolving === undefined ? t("web.action.recordDecision") : t("web.action.resolve")} description={t("web.governance.immutable")} icon={<ClipboardCheck size={16} />} onClose={onClose} footer={<><Button onClick={onClose}>{t("web.action.cancel")}</Button><Button form="governance-event" type="submit" variant="primary">{t("web.action.confirm")}</Button></>}>
