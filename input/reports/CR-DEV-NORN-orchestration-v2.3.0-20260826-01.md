...
| Packaging | PASS — `arka-norn@2.3.0`, 727 fichiers, 2,0 MB compressé ; l’empreinte finale est produite et vérifiée par le workflow du tag |
...
La première matrice distante a en outre révélé des hypothèses POSIX que les contrôles locaux ne montraient pas. Le candidat corrige désormais la résolution de Git sous Windows sans réintroduire la configuration globale, utilise des répertoires privés portables pour les hooks et la configuration Git, résout les shims npm Windows vers leur entrée Node bornée sans `shell`, exécute les wrappers JavaScript via `process.execPath`, et transporte les répertoires temporaires et variables système minimales dans le HOME isolé. Les allocations de worktrees et les écritures de commit du broker sont sérialisées dans le dépôt commun sans réduire le parallélisme des workers. Le watcher Web utilise le polling sous Windows afin d’éviter l’arrêt natif `fs-event` de libuv pendant les remplacements atomiques. Les assertions de chemins, de modes de fichier et de cycle de vie TUI respectent les séparateurs, les ACL et la fin des écritures auditées Windows. La suite locale post-correction est de nouveau PASS — 304/304.
