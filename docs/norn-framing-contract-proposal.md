...
The canonical runtime types live in [`src/domain/framing/framing-plan.ts`](../src/domain/framing/framing-plan.ts). JSON Schema consumers use:
...
The model cannot write derived state, stabilization records, publication authority, or fingerprints. [`assertPlan`](../src/domain/framing/framing-plan.ts) recomputes and verifies those fields after every read and mutation.
...
The adapter in [`fs-repository-probe.ts`](../src/adapters/outbound/filesystem/fs-repository-probe.ts) counts files independently of bounded display samples and ignores build outputs, dependencies, Git metadata, Norn metadata, and GitNexus caches.
...
[`fs-framing-store.ts`](../src/adapters/outbound/filesystem/fs-framing-store.ts) stores mutable work outside the product repository:
...
They begin after framing. [`orchestration-v23-plan-builder.ts`](../src/composition/orchestration-v23-plan-builder.ts) accepts only an exact, signed, grounded Feature plan and maps its Lots to task plans. A missing, tampered, unpublished, or divergent plan blocks orchestration.
...
- [`tests/unit/framing-plan.test.ts`](../tests/unit/framing-plan.test.ts);
- [`tests/integration/framing-engine.test.ts`](../tests/integration/framing-engine.test.ts);
- [`tests/e2e/framing-cli.test.ts`](../tests/e2e/framing-cli.test.ts);
- [`web/src/views/framing-view.test.tsx`](../web/src/views/framing-view.test.tsx).
