...
Le profil `all` installe exactement 21 skills. `arka-norn` initialise le Product principal, `arka-audit` conduit les découvertes transverses sans Pipeline, `arka-product` organise les rôles et la reprise, `arka-essentiel` exécute une phase de Feature guidée, `arka-fastdev` exécute une phase de rework, `arka-framework-maitrise` guide une session standard spécialisée et `arka-git-steward` garde l'hygiène Git des runs multi-agents. Toutes interdisent de deviner Project, Feature, identité ou étape.
...
`arka-audit` conduit une découverte transverse dans la session courante, propose les domaines après `audit inspect`, puis produit un rapport et une KB locale sans Feature ni Pipeline. `arka-framework-audit` impose observation directe et absence de correction silencieuse uniquement lorsque `pipeline next` attend `audit_etat_reel`; il produit alors un audit v3 pour une Feature ou l’audit Project v4 explicite avec `project_id`. Aucun des deux ne fabrique de Feature de convenance. Le skill dev impose lecture de la spec, scope fichiers, tests, CR de dev et handoff. Le skill QA cible le dernier CR, conserve les preuves et sépare structure, verdict métier, anomalies et décision.

`arka-essentiel` et `arka-fastdev` partagent la même discipline guidée : lire exactement l'action calculée par la CLI, conserver la session spécialisée, produire un seul document signé, le valider puis s'arrêter. Essentiel est le défaut des nouvelles Features au périmètre connu et escalade vers Standard si son cadrage révèle une architecture nouvelle, une migration critique ou une forte incertitude. FastDev reste réservé aux reworks bornés.
