...
test("le menu TUI Product ouvre le prompt du rôle calculé avec session et permissions explicites", async () => {
  ...
  const orchestration: ForAgentOrchestration = {
    advise: async () => ({
      ...
      recommendations: [{ role: "audit", mode: "execute", canWrite: true, sessionId: "audit-feature", skill: "arka-fastdev", skillProfile: "audit", reason: "audit requis", command: "arka-norn agent prompt audit" }],
