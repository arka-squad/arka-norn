...
test("dry-run ne modifie pas le marker et apply crée un backup avant migration", async (context) => {
  ...
  const migrated = JSON.parse(readFileSync(markerPath, "utf8")) as { readonly schemaVersion: number; readonly orchestrationMode: string };
...
test("les stores écrivent un marker Project v4 et un marker Feature v3 portables", async (context) => {
  const sandbox = mkdtempSync(join(tmpdir(), "arka-norn-v4-stores-"));
  context.after(() => rmSync(sandbox, { recursive: true, force: true }));
  ...
  const project = Project.create({ id: projectId, name: "Arka Norn", root: projectRoot, schemaVersion: 4, orchestrationMode: "automatic", createdAt: timestamp, updatedAt: timestamp });
  ...
  const projectMarker = JSON.parse(readFileSync(resolve(projectRoot, ".arka-norn", "project.json"), "utf8")) as { readonly schemaVersion: number; readonly orchestrationMode: string };
  ...
  assert.equal(projectMarker.schemaVersion, 4);
  assert.equal(projectMarker.orchestrationMode, "automatic");
...
test("un marker Project v1 reste lisible puis save matérialise project.json v4 manuel", async (context) => {
  ...
  const persisted = JSON.parse(readFileSync(resolve(markerDir, "project.json"), "utf8")) as { readonly schemaVersion: number; readonly orchestrationMode: string };
  assert.equal(persisted.schemaVersion, 4);
  assert.equal(persisted.orchestrationMode, "manual");
});

test("un marker Project v4 déplacé dérive sa nouvelle racine de son emplacement", async (context) => {
  ...
  await store.init(Project.create({
    ...
    schemaVersion: 4, orchestrationMode: "manual", createdAt: timestamp, updatedAt: timestamp,
  }));
...
test("un marker Project v3 reste lisible sans écriture puis save le matérialise en v4 manuel", async (context) => {
  const sandbox = mkdtempSync(join(tmpdir(), "arka-norn-v3-project-"));
  context.after(() => rmSync(sandbox, { recursive: true, force: true }));
  const markerDir = resolve(sandbox, ".arka-norn");
  mkdirSync(markerDir, { recursive: true });
  const markerPath = resolve(markerDir, "project.json");
  copyFileSync(resolve(FIXTURES, "project-marker-v3.json"), markerPath);
  const original = readFileSync(markerPath, "utf8");
  ...
  await store.save(project);
  const persisted = JSON.parse(readFileSync(markerPath, "utf8")) as { readonly schemaVersion: number; readonly orchestrationMode: string };
  assert.equal(persisted.schemaVersion, 4);
  assert.equal(persisted.orchestrationMode, "manual");
});
