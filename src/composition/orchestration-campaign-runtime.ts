...
import type { OrchestrationMissionPlanner } from "./orchestration-mission-planner.js";
import { isActive, nextExecutionId, nextMissionId } from "./orchestration-runtime-support.js";

export interface CampaignRuntime {
  update(project: Project, campaignId: string, transform: (campaign: OrchestrationCampaign) => OrchestrationCampaign): Promise<OrchestrationCampaign>;
  featureInWorkspace(project: Project, feature: Feature, campaign: OrchestrationCampaign): Promise<Feature>;
  advance(project: Project, executionId: string, workspaceFeature: Feature): Promise<void>;
  continueOrBlock(project: Project, campaign: OrchestrationCampaign, stepId: string): Promise<ExecutionRecord | undefined>;
  validateContinuation(project: Project, campaign: OrchestrationCampaign): Promise<void>;
  cleanupUnstarted(project: Project, campaign: OrchestrationCampaign): Promise<void>;
  requestDecision(project: Project, campaign: OrchestrationCampaign, reason: string): Promise<OrchestrationCampaign>;
  requestRetry(project: Project, campaign: OrchestrationCampaign, reason: string): Promise<OrchestrationCampaign>;
  reconcileFailure(project: Project, executionId: string): Promise<void>;
}
...
export function createCampaignRuntime(input: {
  ...
  readonly missionPlanner: OrchestrationMissionPlanner;
  readonly clock: Clock;
  readonly launch: (project: Project, execution: ExecutionRecord) => Promise<ExecutionRecord>;
}): CampaignRuntime {
  async function update(project: Project, campaignId: string, transform: (campaign: OrchestrationCampaign) => OrchestrationCampaign): Promise<OrchestrationCampaign> {
  ...
  async function featureInWorkspace(project: Project, feature: Feature, campaign: OrchestrationCampaign): Promise<Feature> {
    if (campaign.workspaceMode === "direct") return feature;
    ...
    if (featureRelativePath === "" || featureRelativePath === ".." || featureRelativePath.startsWith("../") || featureRelativePath.startsWith("..\\")) throw new Error("The Feature is outside the Product root.");
  ...
  async function advance(project: Project, executionId: string, workspaceFeature: Feature): Promise<void> {
    const campaign = (await input.campaignStore.load(project)).find((candidate) => candidate.missionIds.includes(executionId));
    if (campaign === undefined || campaign.status !== "running") return;
    ...
    if (next === undefined) {
      ...
      await update(project, campaign.id, (candidate) => candidate.requireAction(
        "apply_changes",
        `${changes.changes.length} verified workspace change(s) are ready for human validation.`,
        changes.fingerprint,
        input.clock.now(),
      ));
      return;
    ...
    if (next.decisionGate !== "continue") {
      await update(project, campaign.id, (candidate) => candidate.requireAction(
        "business_decision",
        `A human decision is required before Pipeline step ${next.stepId}.`,
        actionFingerprint(candidate, "business_decision", next.stepId),
        input.clock.now(),
        "awaiting_decision",
        next.stepId,
      ));
  ...
  async function continueOrBlock(project: Project, campaign: OrchestrationCampaign, stepId: string): Promise<ExecutionRecord | undefined> {
    try {
      return await scheduleContinuation(project, campaign, stepId);
    } catch (error) {
      await update(project, campaign.id, (current) => current.requireAction(
        "inspect",
        `The campaign could not continue: ${boundedFailure(error)}`,
        actionFingerprint(current, "inspect", stepId),
        input.clock.now(),
        "blocked",
        stepId,
      ));
  ...
  async function scheduleContinuation(project: Project, campaign: OrchestrationCampaign, stepId: string): Promise<ExecutionRecord | undefined> {
    if (campaign.missionIds.length >= campaign.maxMissions) {
      await update(project, campaign.id, (current) => current.requireAction(
        "inspect",
        "The confirmed campaign mission budget is exhausted; a new preview is required.",
        actionFingerprint(current, "inspect", stepId),
        input.clock.now(),
        "blocked",
        stepId,
    ...
    if (role === undefined) throw new Error(`Pipeline step ${stepId} has no bounded assistant role.`);
    const requirements = requirementsForExecution(role);
    const policy = await input.policyStore.load(project);
    if (policy === undefined || !policy.allowsTarget(campaign.target, requirements)) throw new Error("The campaign target is no longer allowed by Project policy.");
    const registry = await input.registryStore.load(project);
    const active = registry.executions.find(isActive);
    if (active !== undefined) throw new Error(`Execution ${active.id} is still active.`);
  ...
  async function validateContinuation(project: Project, campaign: OrchestrationCampaign): Promise<void> {
    const currentProject = await input.projects.show(project.id);
    if (currentProject.orchestrationMode !== "automatic") throw new Error("Automatic orchestration is disabled; a new preview is required.");
    if (campaign.frameworkVersion !== input.frameworkVersion) throw new Error("The arka.norn framework changed; a new campaign preview is required.");
    const policy = await input.policyStore.load(project);
    ...
    if (currentRuntime?.healthy !== true) throw new Error("The confirmed CLI provider is no longer available; a new preview is required.");
  ...
  async function cleanupUnstarted(project: Project, campaign: OrchestrationCampaign): Promise<void> {
    const current = (await input.campaignStore.load(project)).find((candidate) => candidate.id === campaign.id);
    if (current?.status !== "planned" || current.missionIds.length !== 0) return;
  ...
  function requestDecision(project: Project, campaign: OrchestrationCampaign, reason: string): Promise<OrchestrationCampaign> {
  ...
  function requestRetry(project: Project, campaign: OrchestrationCampaign, reason: string): Promise<OrchestrationCampaign> {
    const exhausted = campaign.retryCount >= 1;
    return update(project, campaign.id, (current) => current.requireAction(
      exhausted ? "inspect" : "retry",
      exhausted ? `The single campaign retry is exhausted. ${reason}` : reason,
      actionFingerprint(current, exhausted ? "inspect" : "retry", current.currentStepId),
      input.clock.now(),
      "blocked",
    ));
  }

  async function reconcileFailure(project: Project, executionId: string): Promise<void> {
    const record = (await input.registryStore.load(project)).find(executionId);
    if (record === undefined || !["failed", "cancelled", "interrupted", "rejected"].includes(record.status)) return;
...
function requireCampaign(campaigns: readonly OrchestrationCampaign[], campaignId: string): OrchestrationCampaign {
...
function actionFingerprint(campaign: OrchestrationCampaign, kind: string, stepId: string): string {
