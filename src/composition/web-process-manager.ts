...
import { FsWebServerStateStore, type WebServerState } from "../adapters/outbound/filesystem/fs-web-server-state-store.js";
...
export interface WebProcessContext {
...
export interface WebProcessStatus {
...
export class WebProcessManager {
  private readonly store: FsWebServerStateStore;

  public constructor(private readonly context: WebProcessContext) {
    this.store = new FsWebServerStateStore(context.homeDir);
  }

  public start(port?: number): Promise<WebProcessStatus> {
    return this.store.exclusive(async () => this.startUnlocked(port));
  }

  public stop(): Promise<WebProcessStatus> {
    return this.store.exclusive(async () => this.stopUnlocked());
  }

  public restart(port?: number): Promise<WebProcessStatus> {
  ...
  public status(): Promise<WebProcessStatus> {
    return this.store.exclusive(async () => this.inspectUnlocked());
  }

  public foreground(port?: number): Promise<WebProcessStatus> {
    return this.store.exclusive(async () => {
      const current = await this.inspectUnlocked();
      if (current.status !== "stopped") throw new Error(this.alreadyRunningMessage(current));
  ...
  private async startUnlocked(port?: number, token?: string): Promise<WebProcessStatus> {
    ...
    child.unref();
    if (child.pid === undefined) throw new Error("Norn Web did not create a server process.");
    for (let elapsed = 0; elapsed < START_TIMEOUT_MS; elapsed += 100) {
      if (spawnError !== undefined) throw spawnError;
      const state = await this.store.load();
      if (state?.pid === child.pid && await probe(state)) return running(state, this.store.logPath());
      if (!isProcessAlive(child.pid)) throw new Error(`Norn Web exited during startup. Inspect ${this.store.logPath()}.`);
      await delay(100);
    }
  ...
  private async stopUnlocked(): Promise<WebProcessStatus> {
    const current = await this.inspectUnlocked();
    if (current.status === "stopped") return current;
    if (current.status === "unresponsive") throw new Error(`Norn Web is unresponsive. Refusing to signal an unverified PID; inspect ${current.logPath}.`);
    await this.stopRunning(current);
    return stopped(this.store.logPath());
  }

  private async stopRunning(current: WebProcessStatus): Promise<void> {
    const pid = current.pid;
    if (pid === undefined) throw new Error("Norn Web state has no PID.");
  ...
  private async inspectUnlocked(): Promise<WebProcessStatus> {
    const state = await this.store.load();
    if (state === undefined) return stopped(this.store.logPath());
    if (await probe(state)) return running(state, this.store.logPath());
  ...
  private alreadyRunningMessage(status: WebProcessStatus): string {
...
async function probe(state: WebServerState): Promise<boolean> {
  try {
...
function running(state: WebServerState, logPath: string): WebProcessStatus {
  return { status: "running", pid: state.pid, port: state.port, url: state.url, startedAt: state.startedAt, cwd: state.cwd, logPath };
}

function stopped(logPath: string): WebProcessStatus {
