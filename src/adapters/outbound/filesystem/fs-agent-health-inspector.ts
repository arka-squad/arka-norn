...
export class FsAgentHealthInspector {
  ...
  private async invalidSelections(selections: readonly (readonly [string, string])[]): Promise<readonly string[]> {
    if (selections.length === 0) return [];
    const raw = await readRaw(join(this.home, ".arka-norn", "index", "projects.json")).catch(() => undefined);
    if (raw === undefined) return selections.map(([projectId]) => `${projectId}:project-not-indexed`);
    let index: unknown;
    try {
    ...
    if (!isProjectIndexFile(index)) return selections.map(([projectId]) => `${projectId}:project-index-invalid`);
    const projectStore = new FsProjectStore();
    const registryStore = new FsAgentRegistryStore();
    const checks = await Promise.all(selections.map(async ([projectId, agentId]) => {
      ...
      if (entry === undefined) return `${projectId}:project-not-indexed`;
      try {
        const project = await projectStore.load(entry.root);
        if (project.id.value !== projectId) return `${projectId}:marker-id-mismatch`;
        if (await readRaw(agentRegistryPath(entry.root)).catch(() => undefined) === undefined) return `${projectId}:registry-missing`;
        const agent = (await registryStore.load(project)).find((candidate) => candidate.id.value === agentId);
        if (agent === undefined) return `${projectId}:${agentId}-missing`;
