...
import { assertPlan, framingPlanFingerprint, type FramingPlan } from "../../../domain/framing/framing-plan.js";
import type { ProjectDraft } from "../../../domain/project/project-draft.js";
import { ProjectId } from "../../../domain/project/project-id.js";
import {
  createProjectPublicationJournal, parseProjectPublicationJournal,
  type ProjectPublicationJournal, type ProjectPublicationState,
} from "../../../domain/project/project-publication.js";
import type { FramingStore, PublishedFramingPlan } from "../../../ports/outbound/framing-store.js";
...
import type {
  ProjectPublicationInspection, ProjectPublicationStore,
} from "../../../ports/outbound/project-publication-store.js";

import { readJson, writeJsonAtomic } from "./_shared/atomic-json.js";
import { withFileLock } from "./_shared/file-lock.js";

type InterruptState = ProjectPublicationState | undefined;

export class FsProjectPublicationStore implements ProjectPublicationStore {
  public constructor(private readonly dependencies: {
    readonly homeDir: string;
    readonly drafts: ProjectDraftStore;
    readonly framing: FramingStore;
    readonly projectIndex: ProjectIndexStore;
    readonly interruptAfter?: InterruptState;
  }) {}

  public async publish(input: { readonly draft: ProjectDraft; readonly plan: FramingPlan; readonly now: Date }): Promise<PublishedFramingPlan> {
    ...
    return withFileLock(path, async () => {
      ...
      try {
        ...
        if (error instanceof PublicationInterruptedError) throw error;
  ...
  public async list(): Promise<readonly ProjectPublicationJournal[]> {
    let entries: Dirent[];
    try {
    ...
    const journals: ProjectPublicationJournal[] = [];
  ...
  public async inspect(projectId: string): Promise<ProjectPublicationInspection> {
    ...
    try {
      ...
      if (!artifacts.safe) return { journal, healthy: false, recoverable: false, message: artifacts.message };
  ...
  public async recover(projectId: string, now: Date): Promise<ProjectPublicationJournal> {
    const journal = await this.requiredJournal(projectId);
    const inspection = await this.inspect(projectId);
    if (inspection.healthy) return journal;
    if (!inspection.recoverable) throw new Error(`Project publication cannot be recovered safely: ${inspection.message}.`);
    const draft = await this.dependencies.drafts.verify(projectId);
    const plan = await this.loadJournalPlan(journal);
  ...
  private async continuePublication(
    draft: ProjectDraft,
    plan: FramingPlan,
    initial: ProjectPublicationJournal,
    now: Date,
  ): Promise<PublishedFramingPlan> {
    let journal = initial;
    if (journal.state === "materialized") return this.publishedResult(journal, plan);
    ...
    if (committedPlan !== undefined) await assertPlanAt(join(journal.root, ...journal.relativePlanPath.split("/")), plan);
    ...
    let published: PublishedFramingPlan;
  ...
  private async transition(journal: ProjectPublicationJournal, state: ProjectPublicationState, now: Date): Promise<ProjectPublicationJournal> {
  ...
  private interrupt(state: ProjectPublicationState): void {
    if (this.dependencies.interruptAfter === state) throw new PublicationInterruptedError(state);
  }

  private async publishedResult(journal: ProjectPublicationJournal, plan: FramingPlan): Promise<PublishedFramingPlan> {
  ...
  private async loadJournalPlan(journal: ProjectPublicationJournal): Promise<FramingPlan> {
    const plan = await this.dependencies.framing.loadRevision(journal.projectId, journal.framingId, journal.planRevision);
    if (plan === undefined) throw new Error("Publication framing revision is missing from the private journal.");
    return plan;
  }

  private async load(projectId: string): Promise<ProjectPublicationJournal | undefined> {
    if (!safeProjectId(projectId)) throw new Error("Invalid Project publication identifier.");
    const value = await readJson<unknown>(journalPath(this.dependencies.homeDir, projectId));
    return value === undefined ? undefined : parseProjectPublicationJournal(value);
  }

  private async requiredJournal(projectId: string): Promise<ProjectPublicationJournal> {
    const journal = await this.load(projectId);
    if (journal === undefined) throw new Error(`Project publication journal not found: ${projectId}.`);
    return journal;
  }
}

class PublicationInterruptedError extends Error {
  public constructor(state: ProjectPublicationState) { super(`Simulated publication interruption after ${state}.`); }
}

function createJournal(draft: ProjectDraft, plan: FramingPlan, now: Date): ProjectPublicationJournal {
...
function assertPublishable(draft: ProjectDraft, plan: FramingPlan): void {
  assertPlan(plan);
  if (plan.stabilizations.groundedPlan === null) throw new Error("Project publication requires the second stabilization.");
  if (plan.target.projectId !== draft.id) throw new Error("Project publication target does not match its draft.");
}

function assertSamePublication(journal: ProjectPublicationJournal, draft: ProjectDraft, plan: FramingPlan): void {
...
async function stageArtifacts(journal: ProjectPublicationJournal, draft: ProjectDraft, plan: FramingPlan): Promise<void> {
  await assertPublicationRootAvailable(journal.root, journal, draft);
  ...
  await writeOrVerify(join(staging, "plan.json"), plan, (value) => {
    try { assertPlan(value as FramingPlan); return framingPlanFingerprint(value as FramingPlan) === journal.planFingerprint; } catch { return false; }
  });
...
async function commitPlan(journal: ProjectPublicationJournal, plan: FramingPlan): Promise<void> {
...
async function commitMarker(journal: ProjectPublicationJournal, draft: ProjectDraft): Promise<void> {
  ...
  await linkExclusive(join(stagingRoot(journal), "project.json"), destination, async () => {
    const current = await readJson<unknown>(destination);
    if (!sameJson(current, marker)) throw publicationError("concurrent_marker", "A concurrent Project marker already exists.");
  });
}
...
async function inspectArtifacts(
  ...
  draft: ProjectDraft,
  plan: FramingPlan,
  index: ProjectIndexStore,
): Promise<{ readonly safe: boolean; readonly complete: boolean; readonly message: string }> {
  try {
    ...
    const publishedPlan = await readJson<FramingPlan>(planPath);
...
async function assertPublicationRootAvailable(root: string, journal?: ProjectPublicationJournal, draft?: ProjectDraft): Promise<void> {
  const canonical = await fs.realpath(resolve(root));
  if (canonical !== resolve(root)) throw new Error("Project publication root changed before publication.");
  const rootStat = await fs.lstat(canonical);
  if (!rootStat.isDirectory() || rootStat.isSymbolicLink()) throw publicationError("unsafe_root", "Project publication root must be a real directory.");
  ...
  for (const marker of [join(canonical, ".arka-norn", "project.json"), join(canonical, ".arka-norn", "depot.json")]) {
    ...
    if (stat.isSymbolicLink()) throw publicationError("symlink", "A symbolic-link marker destination is forbidden.");
    ...
    const current = await readJson<unknown>(marker);
  ...
  if (journal !== undefined) await ensureRealDirectories(canonical, [".arka-norn"]);
}

async function rejectGitSubmodule(root: string): Promise<void> {
  const gitFile = join(root, ".git");
  const stat = await lstatIfPresent(gitFile);
  if (stat === undefined || stat.isDirectory()) return;
  if (stat.isSymbolicLink()) throw publicationError("symlink", "A symbolic-link Git boundary is forbidden.");
...
async function ensureRealDirectories(root: string, segments: readonly string[]): Promise<string> {
  let current = root;
  for (const segment of segments) {
    current = join(current, segment);
    const stat = await lstatIfPresent(current);
    if (stat === undefined) await fs.mkdir(current, { mode: 0o700 });
    else if (stat.isSymbolicLink() || !stat.isDirectory()) throw publicationError("symlink", `Unsafe publication directory: ${relative(root, current)}.`);
  }
  return current;
}

async function writeOrVerify(path: string, value: unknown, verify: (current: unknown) => boolean): Promise<void> {
  try {
    await writeJsonAtomic(path, value, { mode: 0o644, exclusive: true });
  } catch (error) {
    if (!isNodeError(error, "EEXIST")) throw error;
    if (!verify(await readJson<unknown>(path))) throw publicationError("staging_conflict", "Staging contains different publication content.");
  }
}

async function linkExclusive(source: string, destination: string, verify: () => Promise<void>): Promise<void> {
  const sourceStat = await lstatIfPresent(source);
  if (sourceStat?.isSymbolicLink()) throw publicationError("symlink", "Symbolic-link staging artifacts are forbidden.");
  try {
    if (sourceStat === undefined) return verify();
...
async function assertPlanAt(path: string, expected: FramingPlan): Promise<void> {
  const value = await readJson<FramingPlan>(path);
  if (value === undefined) throw new Error("Published framing plan is missing.");
  assertPlan(value);
  if (framingPlanFingerprint(value) !== framingPlanFingerprint(expected)) throw publicationError("plan_conflict", "Published framing plan differs from the authorized revision.");
}

function expectedMarker(draft: ProjectDraft, publishedAt: string): Readonly<Record<string, unknown>> {
  return {
    schemaVersion: 4,
    id: draft.id,
    name: draft.name,
...
async function writeJournal(path: string, journal: ProjectPublicationJournal, exclusive = false): Promise<void> {
...
function publicationError(code: string, message: string): Error {
...
function stagingRoot(journal: ProjectPublicationJournal): string {
  return join(journal.root, ".arka-norn", ".staging", journal.id);
}

function publicationsRoot(homeDir: string): string {
  return join(homeDir, ".arka-norn", "framing-projects");
}

function journalPath(homeDir: string, projectId: string): string {
  if (!safeProjectId(projectId)) throw new Error("Invalid Project publication identifier.");
...
function validTimestamp(value: Date): string {
  if (Number.isNaN(value.getTime())) throw new Error("Invalid Project publication timestamp.");
...
async function syncDirectory(directory: string): Promise<void> {
  ...
  try {
    ...
    if (!isNodeError(error, "EINVAL") && !isNodeError(error, "ENOTSUP") && !(process.platform === "win32" && isNodeError(error, "EPERM"))) throw error;
  } finally {
