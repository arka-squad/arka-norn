...
export class FsPipelineDocumentSource implements PipelineDocumentSource {
  private readonly frameworkRoot: string;
  private readonly paths = new FsPathPolicy();
  ...
  public async list(featureRoot: string): Promise<readonly PipelineDocumentCandidate[]> {
    ...
    const candidates: PipelineDocumentCandidate[] = [];
  ...
  public async read(filePath: string): Promise<PipelineDocumentCandidate> {
    try {
  ...
  public async loadSchema(schemaPath: string): Promise<Readonly<Record<string, unknown>>> {
    const schema = await readJsonObject(resolve(this.frameworkRoot, schemaPath));
    if (schema === undefined) throw new Error(`Schema ${schemaPath} must contain a JSON object.`);
    return schema;
  }
...
function parseTransversalDocuments(value: unknown) {
  ...
  return Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([type, definition]) => {
    ...
    const fields = definition as Readonly<Record<string, unknown>>;
    return { type, schemaPath: requireString(fields["schema"], `pipeline.transversal.${type}.schema`) };
  });
}

async function readJsonObject(filePath: string): Promise<Readonly<Record<string, unknown>> | undefined> {
  const raw = await readRaw(filePath, MAX_JSON_BYTES);
  if (raw === undefined) throw new Error(`File not found: ${filePath}`);
