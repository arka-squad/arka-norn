...
export class LocalAuditCollector implements AuditCollector {
  ...
  public async collect(moduleId: AuditModuleId, context: AuditCollectorContext): Promise<AuditModuleResult> {
    ...
    if (missing.length > 0) limitations.push(`Sources non détectées : ${missing.join(", ")}.`);
    if (context.inspection.workspaceClean === false) limitations.push("L'arbre de travail est modifié ; les observations portent sur un état non commité.");
...
async function collectImportedSources(moduleId: AuditModuleId, context: AuditCollectorContext, observedAt: string, evidenceOffset: number): Promise<CollectionOutcome> {
  ...
  for (const path of context.request.sources.paths) {
    try {
      ...
      if (!stat.isFile() || stat.isSymbolicLink() || stat.size > MAX_TEXT_BYTES) throw new Error("fichier absent, symbolique ou trop volumineux");
      if (!/\.(?:json|md|ya?ml|csv|txt)$/i.test(path)) throw new Error("format d'import non autorisé");
...
function plannedTools(moduleId: AuditModuleId, depth: AuditDepth, inspection: AuditInspection): readonly PlannedTool[] {
  if (depth === "inventaire") return [];
  if (moduleId === "M02" && depth === "dynamique") return runnerTools(inspection);
  ...
  if (moduleId === "M10" && depth === "dynamique") return [{ id: "terraform", arguments: ["validate", "-json"], network: false, writableWorkspace: true }];
  return [];
}
...
async function fetchExternalEvidence(
  ...
  for (let redirect = 0; redirect <= 3; redirect += 1) {
    ...
    if (location === null || redirect === 3) throw new Error("redirection refusée");
    current = new URL(location, current);
  }
  if (response === undefined) throw new Error("réponse absente");
...
async function assertPublicAllowedUrl(url: URL, allowedHosts: readonly string[]): Promise<void> {
  ...
  if (url.protocol !== "https:" && url.protocol !== "http:") throw new Error("protocole non autorisé");
  if (!allowedHosts.includes(host)) throw new Error("hôte absent de l'allowlist");
  if (host === "localhost" || host.endsWith(".localhost") || host === "metadata.google.internal") throw new Error("hôte local ou metadata refusé");
  const addresses = isIP(host) === 0 ? await lookup(host, { all: true, verbatim: true }) : [{ address: host, family: isIP(host) }];
  if (addresses.length === 0 || addresses.some((entry) => isPrivateAddress(entry.address))) throw new Error("résolution vers une adresse privée, link-local ou metadata refusée");
}
...
function suggestedDepth(moduleId: AuditModuleId, state: AuditModuleRecommendation["state"]): AuditDepth {
  if (state === "non_applicable_probable") return "inventaire";
  if (["M05", "M06", "M07", "M08"].includes(moduleId)) return "connecte";
  return "statique";
}
