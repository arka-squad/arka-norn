...
import type {
  ...
  AuditInspection,
  AuditModuleId,
  AuditModuleRecommendation,
  AuditModuleResult,
  AuditSignal,
} from "../../../domain/audit/audit-types.js";
import type { AuditCollector, AuditCollectorContext } from "../../../ports/outbound/audit-collector.js";
import type { AuditToolRunner, AuditToolStatus } from "../../../ports/outbound/audit-tool-runner.js";
...
export class LocalAuditCollector implements AuditCollector {
  public constructor(
    private readonly toolRunnerFactory: (runtime: "docker" | "podman") => AuditToolRunner = (runtime) => new ContainerAuditToolRunner(runtime),
  ) {}

  public doctorTools(runtime: "docker" | "podman", toolIds: readonly AuditToolId[]): Promise<readonly AuditToolStatus[]> {
  ...
  public async collect(moduleId: AuditModuleId, context: AuditCollectorContext): Promise<AuditModuleResult> {
    ...
    let status: AuditModuleResult["execution"]["status"] = missing.length === 0 ? "complete" : "partial";
    if (moduleId === "M00") status = "complete";
    ...
    const tools: AuditModuleResult["execution"]["tools"][number][] = moduleId === "M01" ? [{ name: "git", version: await gitVersion(), dataVersion: null }] : [];
    ...
    if (gitCollection.limitations.length > 0) status = "partial";
    for (const invocation of toolPlan) requested.push(`tool:${invocation.id}`);
    ...
    if (sandboxCollection.limitations.length > 0) status = "partial";
    ...
    if (declaredGaps.limitations.length > 0) status = "partial";
    ...
    if (connected.limitations.length > 0) status = "partial";
    ...
    if (imported.limitations.length > 0) status = "partial";
    const missingAfterTools = requested.filter((id) => !completed.includes(id));
    if (missing.length > 0) limitations.push(translate("audit.collector.sourcesMissing", { sources: missing.join(", ") }));
    if (context.inspection.workspaceClean === false) limitations.push(translate("audit.collector.workspaceModified"));
...
interface CollectionOutcome {
  ...
  readonly tools: readonly AuditModuleResult["execution"]["tools"][number][];
}

function declaredCoverageGaps(
  moduleId: AuditModuleId,
  depth: AuditDepth,
  context: AuditCollectorContext,
  toolPlan: readonly PlannedTool[],
): Pick<CollectionOutcome, "requested" | "limitations"> {
...
function emptyCollection(): CollectionOutcome {
  return { evidence: [], completed: [], requested: [], limitations: [], tools: [] };
}

async function collectGitMetrics(context: AuditCollectorContext, observedAt: string, evidenceOffset: number): Promise<CollectionOutcome> {
  const commands: readonly { readonly id: string; readonly args: readonly string[]; readonly summarize: (output: string) => string }[] = [
...
function summarizeContributors(output: string): string {
...
function summarizeChurn(output: string): string {
  let additions = 0;
  let deletions = 0;
  let entries = 0;
...
async function collectSandboxTools(
  plan: readonly PlannedTool[],
  moduleId: AuditModuleId,
  depth: AuditDepth,
  context: AuditCollectorContext,
  observedAt: string,
  evidenceOffset: number,
  runnerFactory: (runtime: "docker" | "podman") => AuditToolRunner,
): Promise<CollectionOutcome> {
  if (plan.length === 0) return { evidence: [], completed: [], requested: [], limitations: [], tools: [] };
  ...
  const completed: string[] = [];
  const limitations: string[] = [];
  const tools: AuditModuleResult["execution"]["tools"][number][] = [];
  ...
  for (const invocation of plan) {
    ...
    if (result.status === "pass" || result.status === "findings") completed.push(`tool:${invocation.id}`);
    else limitations.push(`${invocation.id} non exploitable : ${safeToolFailure(result.stderr)}.`);
  }
  return { evidence, completed, requested: [], limitations, tools };
}

async function collectConnectedSources(moduleId: AuditModuleId, depth: AuditDepth, context: AuditCollectorContext, observedAt: string, evidenceOffset: number): Promise<CollectionOutcome> {
...
async function collectImportedSources(moduleId: AuditModuleId, context: AuditCollectorContext, observedAt: string, evidenceOffset: number): Promise<CollectionOutcome> {
  const evidence: AuditEvidence[] = [];
  const completed: string[] = [];
  ...
  for (const path of context.request.sources.paths) {
    try {
      ...
      if (canonicalRelative.startsWith("..") || resolve(canonicalProject, canonicalRelative) !== canonical) throw new Error("source hors Project");
      const stat = await fs.lstat(absolute);
      if (!stat.isFile() || stat.isSymbolicLink() || stat.size > MAX_TEXT_BYTES) throw new Error(translate("audit.collector.fileRejected"));
      if (!/\.(?:json|md|ya?ml|csv|txt)$/i.test(path)) throw new Error(translate("audit.collector.formatRejected"));
...
function plannedTools(moduleId: AuditModuleId, depth: AuditDepth, inspection: AuditInspection): readonly PlannedTool[] {
  if (depth === "inventory") return [];
  if (moduleId === "M02" && depth === "dynamic") return runnerTools(inspection);
  if (moduleId === "M04") return [{ id: "syft", arguments: ["dir:/workspace", "-o", "json"], network: false, writableWorkspace: false }];
  if (moduleId === "M05") {
    const local: PlannedTool[] = [{ id: "gitleaks", arguments: ["dir", "/workspace", "--no-banner", "--report-format", "json", "--report-path", "/dev/stdout"], network: false, writableWorkspace: false }];
  ...
  if (moduleId === "M08") return [{ id: "syft", arguments: ["dir:/workspace", "-o", "spdx-json"], network: false, writableWorkspace: false }];
  if (moduleId === "M10" && depth === "dynamic") return [{ id: "terraform", arguments: ["validate", "-json"], network: false, writableWorkspace: true }];
  return [];
}

function runnerTools(inspection: AuditInspection): readonly PlannedTool[] {
  const manifests = inspection.signals.find((signal) => signal.id === "manifest")?.evidence ?? [];
  const tools: PlannedTool[] = [];
  if (manifests.some((path) => path.endsWith("package.json"))) tools.push({ id: "node", arguments: ["npm", "test", "--if-present"], network: false, writableWorkspace: true });
  if (manifests.some((path) => /(?:pyproject\.toml|requirements[^/]*\.txt)$/.test(path))) tools.push({ id: "python", arguments: ["python", "-m", "pytest", "-q"], network: false, writableWorkspace: true });
  if (manifests.some((path) => path.endsWith("go.mod"))) tools.push({ id: "go", arguments: ["go", "test", "./..."], network: false, writableWorkspace: true });
  if (manifests.some((path) => path.endsWith("Cargo.toml"))) tools.push({ id: "rust", arguments: ["cargo", "test", "--locked"], network: false, writableWorkspace: true });
  if (manifests.some((path) => path.endsWith("pom.xml"))) tools.push({ id: "maven", arguments: ["mvn", "-B", "test"], network: false, writableWorkspace: true });
  if (manifests.some((path) => /build\.gradle(?:\.kts)?$/.test(path))) tools.push({ id: "gradle", arguments: ["gradle", "test", "--no-daemon"], network: false, writableWorkspace: true });
  return tools;
}

function safeToolFailure(value: string): string {
  const line = value.trim().split("\n")[0] ?? translate("audit.collector.unknownError");
  return redact(line).slice(0, 240) || translate("audit.collector.unknownError");
}

function connectedModule(moduleId: AuditModuleId): boolean {
  return ["M04", "M05", "M06", "M07", "M08", "M09", "M10", "M11"].includes(moduleId);
}

async function fetchExternalEvidence(
  sourceUrl: string,
  ...
  for (let redirect = 0; redirect <= 3; redirect += 1) {
    ...
    if (location === null || redirect === 3) throw new Error(translate("audit.collector.redirectRejected"));
    current = new URL(location, current);
  }
  if (response === undefined) throw new Error(translate("audit.collector.responseMissing"));
  if (!response.ok) throw new Error(translate("audit.collector.httpError", { status: response.status }));
  const contentLength = Number(response.headers.get("content-length") ?? "0");
  if (Number.isFinite(contentLength) && contentLength > MAX_TEXT_BYTES) throw new Error("source trop volumineuse");
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (bytes.byteLength > MAX_TEXT_BYTES) throw new Error("source trop volumineuse");
...
function connectorHeaders(host: string, credentialRefs: readonly string[]): Record<string, string> {
  const headers: Record<string, string> = { accept: "application/json,text/plain,text/html;q=0.5", "user-agent": "arka-norn-audit/1" };
...
async function assertPublicAllowedUrl(url: URL, allowedHosts: readonly string[]): Promise<void> {
  const host = url.hostname.toLowerCase();
  if (url.protocol !== "https:" && url.protocol !== "http:") throw new Error(translate("audit.collector.protocolRejected"));
  if (!allowedHosts.includes(host)) throw new Error(translate("audit.collector.hostNotAllowed"));
  if (host === "localhost" || host.endsWith(".localhost") || host === "metadata.google.internal") throw new Error(translate("audit.collector.localHostRejected"));
  const addresses = isIP(host) === 0 ? await lookup(host, { all: true, verbatim: true }) : [{ address: host, family: isIP(host) }];
  if (addresses.length === 0 || addresses.some((entry) => isPrivateAddress(entry.address))) throw new Error(translate("audit.collector.privateAddressRejected"));
}
...
async function buildInventory(projectRoot: string, roots: readonly string[]): Promise<Inventory> {
  const files: string[] = [];
  for (const root of roots) await walk(projectRoot, root, files);
  files.sort();
  const detected = (predicate: (path: string) => boolean): readonly string[] => files.filter(predicate).slice(0, 20);
...
async function readPackageMetadata(projectRoot: string, files: readonly string[]): Promise<{ readonly packageName: string | null; readonly github: string | null }> {
  const packagePath = files.find((path) => path === "package.json" || path.endsWith("/package.json"));
  if (packagePath === undefined) return { packageName: null, github: null };
  const raw = await readBoundedText(join(projectRoot, packagePath));
  if (raw === undefined) return { packageName: null, github: null };
  try {
    const value = JSON.parse(raw) as unknown;
    if (typeof value !== "object" || value === null || Array.isArray(value)) return { packageName: null, github: null };
    ...
    const repositoryValue = typeof record["repository"] === "string"
      ...
      : typeof record["repository"] === "object" && record["repository"] !== null && !Array.isArray(record["repository"])
        ? (record["repository"] as Readonly<Record<string, unknown>>)["url"] : null;
...
async function walk(projectRoot: string, directory: string, files: string[]): Promise<void> {
  if (files.length >= MAX_FILES) return;
  const entries = await fs.readdir(directory, { withFileTypes: true }).catch(() => []);
  for (const entry of entries) {
    if (files.length >= MAX_FILES || entry.isSymbolicLink() || EXCLUDED_SEGMENTS.has(entry.name)) continue;
    const absolute = join(directory, entry.name);
    if (entry.isDirectory()) await walk(projectRoot, absolute, files);
    else if (entry.isFile()) files.push(relative(projectRoot, absolute).replaceAll("\\", "/"));
  }
}

function recommendations(signals: readonly AuditSignal[]): readonly AuditModuleRecommendation[] {
  ...
  return AUDIT_MODULE_CATALOG.filter((definition) => definition.id !== "M00").map((definition) => {
    const matching = definition.signalIds.map((id) => byId.get(id)).filter((item): item is AuditSignal => item !== undefined);
    const found = matching.filter((item) => item.detected).length;
    const state: AuditModuleRecommendation["state"] = found === matching.length && found > 0
      ? "recommended"
...
function suggestedDepth(moduleId: AuditModuleId, state: AuditModuleRecommendation["state"]): AuditDepth {
  if (state === "probably_not_applicable") return "inventory";
  if (["M05", "M06", "M07", "M08"].includes(moduleId)) return "connected";
  return "static";
}

function moduleEvidence(moduleId: AuditModuleId, signals: readonly AuditSignal[], context: AuditCollectorContext, observedAt: string): readonly AuditEvidence[] {
...
async function inspectGit(root: string): Promise<{ readonly commit: string | null; readonly status: string; readonly clean: boolean | null }> {
  try {
...
async function inspectSandbox(): Promise<AuditInspection["sandbox"]> {
...
function containedRoot(projectRoot: string, value: string): string {
  if (value.startsWith("/") || /^[A-Za-z]:[\\/]/.test(value) || value.split(/[\\/]/).includes("..")) throw new Error(`Audit path is outside the Project: ${value}`);
  const target = resolve(projectRoot, value);
  const rel = relative(projectRoot, target);
  if (rel.startsWith("..") || resolve(projectRoot, rel) !== target) throw new Error(`Audit path is outside the Project: ${value}`);
  return target;
}

function signal(id: string, detected: boolean, evidence: readonly string[]): AuditSignal {
...
export async function readBoundedText(path: string): Promise<string | undefined> {
  try {
    ...
    if (!stat.isFile() || stat.size > MAX_TEXT_BYTES) return undefined;
