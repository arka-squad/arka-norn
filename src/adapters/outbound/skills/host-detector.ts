...
export const SUPPORTED_HOSTS: readonly SupportedHost[] = ["codex", "claude"];
...
export interface HostDetectionResult {
  readonly detected: readonly HostDetection[];
  readonly missing: readonly SupportedHost[];
}

export function detectHosts(envPath = process.env.PATH): HostDetectionResult {
  const pathEntries = (envPath ?? "").split(delimiter).filter((entry) => entry.length > 0);
  const detected = new Map<SupportedHost, HostDetection>();
...
export function detectHostsFiltered(hostFilter: SupportedHost | "all", envPath = process.env.PATH): HostDetectionResult {
  if (hostFilter === "all") return detectHosts(envPath);
...
function findExecutable(command: string, pathEntries: readonly string[]): string | undefined {
...
export function formatHosts(hosts: readonly HostDetection[]): string {
