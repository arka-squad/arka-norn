...
export class DirectSkillManager implements SkillManager {
  ...
  public constructor(frameworkRoot: string, globalHome = homedir()) {
  ...
  public inspectGlobal(profile = "all"): Promise<SkillHealth> {
...
function summarize(definitions: readonly { readonly status: "ok" | "missing" | "divergent" }[]): SkillHealth {
  let healthy = 0;
  let missing = 0;
  let divergent = 0;
