...
import type { GitIntegrationResult, GitTaskCommit, GitTaskWorkspace, GitWorkspacePort } from "../../../ports/outbound/git-workspace.js";
...
export class GitWorktreeWorkspaceAdapter implements GitWorkspacePort {
  ...
  public constructor(private readonly homeDir: string, options: { readonly gitCommand?: string } = {}) {
    ...
    if (this.gitCommand !== "git" && !isAbsolute(this.gitCommand)) throw new TypeError("Configured Git command must be absolute.");
    this.worktreeRoot = join(homeDir, ".arka-norn", "worktrees");
    this.gitHome = join(homeDir, ".arka-norn", "git-home");
  }

  public async createSnapshot(project: Project, input: { readonly campaignId: string; readonly includeScopes: readonly string[]; readonly declaredUntracked: readonly string[] }): Promise<BaseSnapshot> {
    ...
    for (const path of input.declaredUntracked) {
      if (!untracked.has(path)) throw new Error(`Declared untracked file is absent or ignored: ${path}`);
      if (!insideAnyScope(path, input.includeScopes)) throw new Error(`Declared untracked file is outside the snapshot scope: ${path}`);
    ...
    for (const path of modifiedTracked) await this.git(root, ["add", "-u", "--", path], { GIT_INDEX_FILE: index });
    for (const path of input.declaredUntracked) await this.git(root, ["add", "-f", "--", path], { GIT_INDEX_FILE: index });
  ...
  public async createTaskWorkspace(project: Project, snapshot: BaseSnapshot, campaignId: string, task: TaskPlan): Promise<GitTaskWorkspace> {
  ...
  public async commitTask(project: Project, workspace: GitTaskWorkspace, task: TaskPlan, input: { readonly campaignId: string; readonly agentId: string; readonly profileId: string; readonly executionId: string; readonly proofReferences: readonly string[] }): Promise<GitTaskCommit> {
    await this.assertRepository(project);
    if (workspace.campaignId !== input.campaignId || workspace.taskId !== task.id) throw new Error("Task workspace identity mismatch.");
    validateId(input.profileId, "profile id");
    validateId(input.executionId, "execution id");
    if (input.agentId !== task.agentId) throw new Error("Task Agent identity mismatch.");
    if (input.proofReferences.length === 0 || input.proofReferences.some((value) => !safeText(value, 512))) throw new Error("Task commit requires bounded proof references.");
    const changedPaths = await this.changedPaths(workspace.path);
    if (changedPaths.length === 0) throw new Error("Task produced no changes to commit.");
    for (const path of changedPaths) {
      if (!insideAnyScope(path, task.writeScopes)) throw new Error(`Task change exceeds its write scope: ${path}`);
  ...
  public async integrate(project: Project, snapshot: BaseSnapshot, campaignId: string, commits: readonly GitTaskCommit[]): Promise<GitIntegrationResult> {
    validateId(campaignId, "campaign id");
    if (commits.length === 0) throw new Error("Integration requires at least one task commit.");
    const root = await this.assertRepository(project);
  ...
  public async resolveIntegrationConflict(project: Project, integration: GitIntegrationResult, input: { readonly agentId: string; readonly profileId: string; readonly executionId: string; readonly proofReferences: readonly string[] }): Promise<GitIntegrationResult> {
    await this.assertRepository(project);
    if (integration.status !== "conflicted" || integration.pendingCommits === undefined || integration.pendingCommits.length === 0) throw new Error("Integration has no resolvable conflict state.");
    validateId(input.profileId, "profile id");
    validateId(input.executionId, "execution id");
    if (input.proofReferences.length === 0) throw new Error("Integration conflict resolution requires proof.");
    for (const path of integration.conflictPaths) await assertSafeChangedPath(integration.path, path);
    ...
    if (unresolved.length > 0) throw new Error(`Integration conflict remains unresolved: ${unresolved.join(", ")}`);
  ...
  public async buildPriorityFallback(project: Project, integration: GitIntegrationResult): Promise<GitIntegrationResult> {
    await this.assertRepository(project);
    if (integration.status !== "conflicted" || integration.pendingCommits === undefined || integration.pendingCommits.length === 0) throw new Error("Integration has no priority fallback state.");
    const discarded: { commit: string; path: string; hunk: string; fingerprint: string }[] = [];
    const record = async (commit: string): Promise<void> => {
      ...
      for (const path of paths) {
        ...
        for (const hunk of hunks.length === 0 ? ["whole-file-change"] : hunks) discarded.push({ commit, path, hunk: hunk.slice(0, 300), fingerprint: hash(`${commit}\n${path}\n${hunk}`) });
      }
    };
  ...
  public async applyFastForward(project: Project, snapshot: BaseSnapshot, integration: GitIntegrationResult): Promise<string> {
    if (!snapshot.clean) throw new Error("Automatic application is forbidden for a dirty base snapshot.");
    if (integration.status !== "integrated" || integration.commit === undefined) throw new Error("Only a conflict-free integration can be applied.");
    const root = await this.assertRepository(project);
    if ((await this.git(root, ["status", "--porcelain=v1"])).stdout.trim() !== "") throw new Error("The real Project is no longer clean.");
    if ((await this.git(root, ["rev-parse", "HEAD"])).stdout.trim() !== snapshot.commit) throw new Error("The real Project changed after the campaign snapshot.");
    const ancestor = await this.git(root, ["merge-base", "--is-ancestor", snapshot.commit, integration.commit], {}, [0, 1]);
    if (ancestor.code !== 0) throw new Error("Integration is not a descendant of the confirmed snapshot.");
  ...
  public async inspectRiskChanges(project: Project, snapshot: BaseSnapshot, integration: GitIntegrationResult, commits: readonly GitTaskCommit[]): Promise<readonly RiskChange[]> {
    await this.assertRepository(project);
    if (integration.status !== "integrated" || integration.commit === undefined) throw new Error("Risk inspection requires an integrated campaign candidate.");
    const declaredPaths = new Set(commits.flatMap((commit) => [...commit.changedPaths]));
    const proofByPath = new Map(commits.flatMap((commit) => commit.changedPaths.map((path) => [path, /^[a-f0-9]{64}$/u.test(commit.evidenceFingerprint)] as const)));
  ...
  private async assertRepository(project: Project): Promise<string> {
    const root = await realpath(project.root);
    const top = (await this.git(root, ["rev-parse", "--show-toplevel"])).stdout.trim();
    if (await realpath(top) !== root) throw new Error("Project root must be the Git worktree root.");
    return root;
  }

  private async assertSafeRepository(root: string): Promise<void> {
    if ((await this.git(root, ["ls-files", "--error-unmatch", ".gitmodules"], {}, [0, 1])).code === 0) throw new Error("Git submodules are forbidden in automatic orchestration snapshots.");
    const filters = await this.git(root, ["config", "--local", "--get-regexp", "^filter\\."], {}, [0, 1]);
    if (filters.code === 0 && filters.stdout.trim() !== "") throw new Error("External Git filters are forbidden in automatic orchestration snapshots.");
  }
  ...
  private async operationFor(root: string, before: string, after: string, path: string): Promise<ChangeOperation> {
  ...
  private async numstatFor(root: string, before: string, after: string, path: string): Promise<{ readonly churn: number; readonly binary: boolean }> {
    const line = (await this.git(root, ["diff", "--numstat", "--no-renames", before, after, "--", path])).stdout.split("\n").find((value) => value.trim() !== "");
    if (line === undefined) return { churn: 0, binary: false };
    const [added, removed] = line.split("\t");
    if (added === "-" || removed === "-") return { churn: 0, binary: true };
    return { churn: Number(added ?? 0) + Number(removed ?? 0), binary: false };
  }

  private async modeFor(root: string, commit: string, path: string): Promise<string | undefined> {
    const result = await this.git(root, ["ls-tree", commit, "--", path]);
    return result.stdout.trim() === "" ? undefined : result.stdout.split(/\s+/u)[0];
  }

  private async boundedBlob(root: string, commit: string, path: string): Promise<string> {
    const size = Number((await this.git(root, ["cat-file", "-s", `${commit}:${path}`])).stdout.trim());
    if (!Number.isSafeInteger(size) || size < 0 || size > 1024 * 1024) return "";
    return (await this.git(root, ["show", `${commit}:${path}`])).stdout;
  }

  private async appendIntegrationTrailers(root: string, input: { readonly agentId: string; readonly profileId: string; readonly executionId: string; readonly proofReferences: readonly string[] }): Promise<void> {
...
async function run(command: string, args: readonly string[], cwd: string, environment: Readonly<Record<string, string>>): Promise<GitResult> {
  return new Promise((resolvePromise, reject) => {
    ...
    let stdout = "";
    let stderr = "";
    const append = (current: string, chunk: Buffer): string => { const next = current + chunk.toString("utf8"); if (Buffer.byteLength(next) > MAX_OUTPUT_BYTES) { child.kill("SIGTERM"); throw new Error("Git output exceeded the safe limit."); } return next; };
    child.stdout.on("data", (chunk: Buffer) => { try { stdout = append(stdout, chunk); } catch (error) { reject(asError(error)); } });
    child.stderr.on("data", (chunk: Buffer) => { try { stderr = append(stderr, chunk); } catch (error) { reject(asError(error)); } });
    child.on("error", (error) => reject(error));
    child.on("close", (code) => resolvePromise({ code: code ?? 1, stdout, stderr }));
  });
}

async function assertSafeRegularFile(root: string, path: string): Promise<void> { const target = safeJoin(root, path); const info = await lstat(target); if (!info.isFile() || info.isSymbolicLink() || info.size > MAX_FILE_BYTES || SECRET_FILE.test(path.split("/").at(-1)!)) throw new Error(`Unsafe declared file: ${path}`); }
async function assertSafeChangedPath(root: string, path: string): Promise<void> { if (path === ".git" || path.startsWith(".git/") || SECRET_FILE.test(path.split("/").at(-1)!)) throw new Error(`Unsafe task change: ${path}`); try { const info = await lstat(safeJoin(root, path)); if (info.isSymbolicLink() || (!info.isFile() && !info.isDirectory())) throw new Error(`Unsafe task change: ${path}`); } catch (error) { if (!isNodeError(error, "ENOENT")) throw error; } }
async function assertNoSecretContent(root: string, path: string): Promise<void> { try { const target = safeJoin(root, path); const info = await lstat(target); if (!info.isFile() || info.size > 1024 * 1024) return; const value = await readFile(target, "utf8"); SECRET_CONTENT.lastIndex = 0; if (SECRET_CONTENT.test(value)) throw new Error(`Task change contains credential-like content: ${path}`); } catch (error) { if (!isNodeError(error, "ENOENT")) throw error; } }
async function assertMissing(path: string): Promise<void> { try { await lstat(path); throw new Error(`Workspace path already exists: ${path}`); } catch (error) { if (isNodeError(error, "ENOENT")) return; throw error; } }
function safeJoin(root: string, path: string): string { const target = resolve(root, path); const relation = relative(root, target); if (relation === ".." || relation.startsWith(`..${sep}`) || isAbsolute(relation)) throw new Error(`Path escapes the workspace: ${path}`); return target; }
function validateScopes(values: readonly string[], allowEmpty = false): void { if ((!allowEmpty && values.length === 0) || new Set(values).size !== values.length || values.some((value) => !safeScope(value))) throw new TypeError("Git workspace scopes are invalid."); }
function safeScope(value: string): boolean { return value === "." || (value.length > 0 && value.length <= 512 && !value.startsWith("/") && !value.includes("\\") && value.split("/").every((part) => part !== "" && part !== "." && part !== "..")); }
function insideAnyScope(path: string, scopes: readonly string[]): boolean { return scopes.some((scope) => scope === "." || path === scope || path.startsWith(`${scope}/`)); }
function validateId(value: string, label: string): void { if (!/^[a-z0-9][a-z0-9._-]{0,119}$/u.test(value)) throw new TypeError(`Invalid ${label}.`); }
function safeText(value: string, maximum: number): boolean { return value.length > 0 && value.length <= maximum && !/[\u0000-\u001f\u007f]/u.test(value); }
function splitZero(value: string): string[] { return value.split("\0").filter((entry) => entry.length > 0); }
function hash(value: string): string { return createHash("sha256").update(value).digest("hex"); }
function executable(mode: string | undefined): boolean { return mode === "100755"; }
function sanitize(value: string): string { return value.replaceAll(/(?:Bearer\s+|sk-)[a-z0-9._-]+/giu, "[REDACTED]").replaceAll(/[\u0000-\u001f\u007f]+/gu, " ").trim().slice(0, 1_000); }
function gitIdentity(name: string): Record<string, string> { return { GIT_AUTHOR_NAME: name, GIT_AUTHOR_EMAIL: "norn@local.invalid", GIT_COMMITTER_NAME: name, GIT_COMMITTER_EMAIL: "norn@local.invalid" }; }
