...
function validatedEnvironmentValue(value: string, label: string): string {
  if (value.length === 0 || value.length > 16 * 1024 || value.includes("\u0000")) throw new Error(`${label} is invalid.`);
  return value;
}
