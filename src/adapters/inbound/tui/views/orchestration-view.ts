...
import type { ForOrchestration, OrchestrationStatus } from "../../../../ports/inbound/for-orchestration.js";
...
type OrchestrationAction = "start" | "refresh" | "cancel" | "approve" | "retry" | "back";
  ...
  function items(): readonly MenuItem<OrchestrationAction>[] {
    ...
    return [
      ...
      ...(active !== undefined && (active.status === "planned" || active.status === "running" || active.status === "awaiting_approval")
        ? [{ label: `Annuler ${active.id}`, value: "cancel" as const, description: "annulation explicite, tracée dans le registre" }]
        : []),
      ...(active?.status === "awaiting_approval"
        ? [{ label: `Approuver ${active.id}`, value: "approve" as const, description: "relance après contrôle des préconditions actuelles" }]
        : []),
      ...(active !== undefined && (active.status === "failed" || active.status === "cancelled" || active.status === "interrupted")
        ? [{ label: `Relancer ${active.id}`, value: "retry" as const, description: "nouvelle tentative avec le même provider, jamais un fallback caché" }]
        : []),
  ...
  return {
    ...
    render(renderer: Renderer, theme: Theme): void {
      renderer.redraw((line) => {
        ...
        line("");
        for (const value of renderExecutionDetails(active, theme)) line(value);
...
function requireActiveExecution(status: OrchestrationStatus): ExecutionRecord {
  if (status.activeExecution === undefined) throw new Error("Aucune mission active à modifier.");
  return status.activeExecution;
}

function renderExecutionDetails(execution: ExecutionRecord | undefined, theme: Theme): readonly string[] {
  if (execution === undefined) return [theme.dim("  Le registre ne contient aucune mission active.")];
