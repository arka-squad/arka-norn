...
export interface StartWebServerOptions {
  ...
  readonly service: ProjectTrackingService;
  readonly management: ConstructorParameters<typeof LiveWatcher>[0];
...
export async function startWebServer(options: StartWebServerOptions): Promise<RunningWebServer> {
  ...
  if (address === null || typeof address === "string") throw new Error("Norn Web did not acquire a TCP port.");
...
async function handleRequest(
  ...
  if (!isLoopback(request.socket.remoteAddress)) return sendError(response, 403, "loopback_required", locale);
  const url = new URL(request.url ?? "/", "http://127.0.0.1");
  try {
    if (url.pathname.startsWith("/api/")) {
      if (!authorizeRequest(request, token, origin())) return sendError(response, 401, "unauthorized", locale);
