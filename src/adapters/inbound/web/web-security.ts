...
export function authorizeRequest(request: IncomingMessage, token: string, expectedOrigin: string): boolean {
  if (!isLoopback(request.socket.remoteAddress)) return false;
  const origin = request.headers.origin;
  const expectedHost = new URL(expectedOrigin).host;
  if (origin === undefined ? request.headers.host !== expectedHost : origin !== expectedOrigin) return false;
