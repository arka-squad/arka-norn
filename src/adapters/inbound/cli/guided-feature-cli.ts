...
export interface GuidedWorkflowCliConfig {
...
export async function runGuidedFeatureCommand(
  argv: readonly string[],
  context: GuidedFeatureCliContext,
  config: GuidedWorkflowCliConfig,
): Promise<CliExecution> {
  const action = argv[0];
  const rest = argv.slice(1);
  const json = rest.includes("--json");
  try {
    if (action === "start") return await start(rest, context, config, json);
    if (action === "status" || action === "next") return await inspect(action, rest, context, config, json);
...
async function inspect(
  ...
  if (feature.pipelineId !== config.pipelineId) throw new CliUsageError(`Feature ${feature.id.value} uses ${feature.pipelineId}, not ${config.displayName}.`);
