...
export async function runWebCommand(argv: readonly string[], context: WebCliContext): Promise<CliExecution> {
  ...
  if (!parsed.noOpen) openBrowser(server.url);
  return { code: 0, stdout: `Norn Web is available at ${server.url}\n`, stderr: "" };
}

function parseArguments(argv: readonly string[]): { readonly port?: number; readonly noOpen: boolean } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--port") {
      ...
      if (!Number.isInteger(candidate) || candidate < 1 || candidate > 65_535) throw new Error("--port must be an integer between 1 and 65535.");
      port = candidate;
      index += 1;
      continue;
    }
