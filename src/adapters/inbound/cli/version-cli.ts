...
import { bootId, evaluateVersionAdvisory, type VersionAdvisory } from "../../../application/version/version-advisory.js";
...
export interface VersionCliContext {
  readonly homeDir: string;
  readonly latestVersion?: () => Promise<string | undefined>;
  readonly nowMs?: number;
  readonly uptimeSeconds?: number;
}

export async function runVersionCommand(argv: readonly string[], context: VersionCliContext): Promise<CliExecution> {
  ...
  try {
    ...
    if (parsed.booleans.has("skip-reboot") || parsed.booleans.has("skip-version")) {
      if (latest === undefined) return envelope("version.skip", { current: PRODUCT_VERSION }, json, [], translate("cli.version.unknown"));
      const kind = parsed.booleans.has("skip-version") ? "version" as const : "reboot" as const;
    ...
    if (json) return { code: error instanceof CliUsageError ? 64 : 70, stdout: cliEnvelopeJson("version", false, { current: PRODUCT_VERSION }, [message]), stderr: "" };
    return { code: error instanceof CliUsageError ? 64 : 70, stdout: "", stderr: `${message}\n` };
  }
}

function envelope(command: string, data: unknown, json: boolean, errors: readonly string[], message: string): CliExecution {
  if (json) return { code: 0, stdout: cliEnvelopeJson(command, errors.length === 0, data, errors, message), stderr: "" };
  return { code: 0, stdout: `${message}\n`, stderr: "" };
}

function cliEnvelopeJson(command: string, ok: boolean, data: unknown, errors: readonly string[], message?: string): string {
  return `${JSON.stringify(cliEnvelope({ command, ok, data, errors, ...(message === undefined ? {} : { message }) }))}\n`;
}

export function renderAdvisory(advisory: VersionAdvisory): string {
...
function advisoryBody(advisory: VersionAdvisory, theme: ReturnType<typeof createTheme>): string {
...
function actionsHint(theme: ReturnType<typeof createTheme>): string {
  return `  ${theme.dim(translate("cli.version.clearHint"))}`;
}

function renderUpdateInstructions(advisory: VersionAdvisory): string {
  const theme = createTheme(process.env, Boolean(process.stdout.isTTY));
