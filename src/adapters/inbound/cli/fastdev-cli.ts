...
export interface FastDevCliContext {
  readonly cwd: string;
...
export async function runFastDevCommand(argv: readonly string[], context: FastDevCliContext): Promise<CliExecution> {
  ...
  try {
    if (action === "start") return await start(rest, context, json);
    if (action === "status" || action === "next") return await inspect(action, rest, context, json);
...
async function start(argv: readonly string[], context: FastDevCliContext, json: boolean): Promise<CliExecution> {
  ...
  if (projectValue === undefined) throw new CliUsageError("fastdev start requires --project <id>");
...
async function inspect(action: "status" | "next", argv: readonly string[], context: FastDevCliContext, json: boolean): Promise<CliExecution> {
  ...
  if (data.action === null) return { code: pipelineExitCode(report), stdout: "FastDev terminé — validation pass sur le dernier CR.\n", stderr: "" };
  return {
    code: pipelineExitCode(report),
    stdout: [
