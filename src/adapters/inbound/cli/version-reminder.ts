...
export interface VersionReminderContext {
 ...
 */
export async function versionReminderLine(context: VersionReminderContext): Promise<string | undefined> {
  ...
  if (advisory.status !== "update_available") return undefined;
  return translate("cli.version.updateAvailableShort", { current: advisory.current, latest: advisory.latest });
}

async function refreshCache(context: VersionReminderContext, store: FsVersionCacheStore, nowMs: number): Promise<void> {
  try {
    const latest = await (context.latestVersion ?? (() => fetchLatestNpmVersion("arka-norn")))();
    if (latest !== undefined) await store.save({ latest, checkedAt: new Date(nowMs).toISOString() });
  } catch {
