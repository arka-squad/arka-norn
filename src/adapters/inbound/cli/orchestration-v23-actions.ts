...
import type { ExecutionCapability } from "../../../domain/orchestration/types.js";
import type { Project } from "../../../domain/project/project.js";
import type { StrictArguments } from "./strict-arguments.js";
import { CliUsageError } from "./strict-arguments.js";

export async function runOrchestrationV23Action(input: {
  readonly action: "profile" | "recovery";
...
function profilePayload(value: OrchestrationConfiguration): unknown { return serializeOrchestrationConfiguration(value); }

async function runProfile(input: Parameters<typeof runOrchestrationV23Action>[0]): Promise<unknown> {
  ...
  if (operation === "register") {
    ...
    if (input.args.booleans.has("activate")) updated = updated.activate(at);
    await store.save(input.project, updated);
    return profilePayload(updated);
  }
  if (operation === "doctor") {
    const configuration = await store.load(input.project);
    if (configuration === undefined) return { schemaVersion: 1, healthy: false, checks: [{ code: "profile_missing", healthy: false, message: "No 2.3 execution profile is configured." }] };
    ...
    if (profiles.length === 0) throw new CliUsageError(`execution profile not found: ${requestedId ?? "unknown"}`);
...
async function runRecovery(input: Parameters<typeof runOrchestrationV23Action>[0]): Promise<unknown> {
  const operation = input.args.positionals[0];
  const recovery = new FsOrchestrationRecovery(input.homeDir);
  if (operation === "inspect") return serializeManifest(await recovery.inspect(input.project));
  if (operation === "quarantine") return serializeReceipt(await recovery.quarantine(input.project, required(input.args, "confirm")));
  const quarantineId = input.args.positionals[1];
  if (quarantineId === undefined) throw new CliUsageError(`recovery ${operation ?? "action"} requires a quarantine id`);
  if (operation === "restore") return serializeReceipt(await recovery.restore(input.project, quarantineId, required(input.args, "confirm")));
...
async function executionProfileFrom(args: StrictArguments, at: Date): Promise<ExecutionProfile> {
  const transport = required(args, "transport");
  const costMeter = args.values.get("cost-meter") ?? "unknown";
  if (!isExecutionTransport(transport)) throw new CliUsageError("--transport must be codex-cli, claude-cli, gemini-cli or api");
  if (!isCostMeterKind(costMeter)) throw new CliUsageError("--cost-meter is invalid");
  const credentialKind = args.values.get("credential-kind");
  if (credentialKind !== undefined && !isCredentialReferenceKind(credentialKind)) throw new CliUsageError("--credential-kind must be environment or keychain");
  ...
  if (gatewayKind === undefined && (catalogRef !== undefined || gatewayEndpoint !== undefined)) throw new CliUsageError("--gateway-kind is required with a gateway endpoint or catalog");
  if (gatewayKind !== undefined && (catalogRef === undefined || gatewayEndpoint === undefined)) throw new CliUsageError("gateway profiles require both --gateway-endpoint and --catalog-ref");
...
function inputEnabled(args: StrictArguments): boolean { return args.booleans.has("enabled") || args.booleans.has("activate"); }
function csv(value: string): string[] { return value === "" ? [] : value.split(",").map((entry) => entry.trim()).filter((entry) => entry.length > 0); }
function csvCapabilities(value: string): ExecutionCapability[] { const values = csv(value); const allowed = new Set<ExecutionCapability>(["inspect_workspace", "modify_workspace", "run_commands", "read_pipeline"]); if (values.some((entry) => !allowed.has(entry as ExecutionCapability))) throw new CliUsageError("--capabilities contains an unsupported value"); return values as ExecutionCapability[]; }
function required(args: StrictArguments, name: string): string { const value = args.values.get(name); if (value === undefined) throw new CliUsageError(`--${name} is required`); return value; }
function serializeManifest(value: Awaited<ReturnType<FsOrchestrationRecovery["inspect"]>>): unknown { return { ...value, entries: value.entries.map((entry) => ({ ...entry })), exactDuplicateAgentGroups: value.exactDuplicateAgentGroups.map((group) => [...group]), inspectedAt: value.inspectedAt.toISOString() }; }
function serializeReceipt(value: Awaited<ReturnType<FsOrchestrationRecovery["quarantine"]>>): unknown { return { ...value, quarantinedAt: value.quarantinedAt.toISOString() }; }
