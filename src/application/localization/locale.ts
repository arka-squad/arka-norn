...
export function resolveLocale(input: LocaleResolutionInput = {}): Locale {
  ...
  if (input.preference !== undefined && input.preference !== "auto") return input.preference;
...
export function parseLocalePreference(value: string): LocalePreference {
  const normalized = value.trim().toLowerCase();
  if (normalized === "auto" || normalized === "en" || normalized === "fr") return normalized;
  throw new Error(`Unsupported locale preference: ${JSON.stringify(value)}. Expected auto, en or fr.`);
}

export function runWithLocale<T>(locale: Locale, operation: () => T): T {
...
export function translate(key: MessageKey, parameters: MessageParameters = {}, locale: Locale = activeLocale()): string {
  const template = CATALOGS[locale][key];
  return template.replace(/\{([A-Za-z][A-Za-z0-9_]*)\}/g, (_placeholder, name: string) => {
    const value = parameters[name];
    if (value === undefined) throw new Error(`Missing translation parameter ${name} for ${key}.`);
    return String(value);
  });
}
...
export function formatDate(value: Date, locale: Locale = activeLocale()): string {
  return new Intl.DateTimeFormat(locale, { dateStyle: "medium" }).format(value);
}

export function formatShortDate(value: Date, locale: Locale = activeLocale()): string {
  return new Intl.DateTimeFormat(locale, { dateStyle: "short" }).format(value);
}

export function formatTime(value: Date, locale: Locale = activeLocale()): string {
  return new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit" }).format(value);
}

export function formatDuration(milliseconds: number, locale: Locale = activeLocale()): string {
...
export function formatBytes(value: number, locale: Locale = activeLocale()): string {
...
export function plural(value: number, singular: string, pluralForm: string, locale: Locale = activeLocale()): string {
...
export function assertCatalogParity(): void {
  ...
  if (JSON.stringify(englishKeys) !== JSON.stringify(frenchKeys)) throw new Error("English and French message keys differ.");
...
function parseExplicitLocale(value: string | undefined, source: string): Locale | undefined {
...
function localeFromLanguageTag(value: string): Locale {
