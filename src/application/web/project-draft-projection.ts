...
import type { FramingSummaryView, ProjectListItem, ProjectOverview } from "./contracts.js";

export function createProjectDraftListItem(draft: ProjectDraft, framing?: FramingSummaryView): ProjectListItem {
  return {
    id: draft.id,
    name: draft.name,
    root: draft.root,
...
export function createProjectDraftOverview(draft: ProjectDraft, framing?: FramingSummaryView): ProjectOverview {
...
function activeMaterialization(value: ProjectDraft["materialization"]): "draft" | "publishing" | "recovery_required" {
  if (value === "materialized") throw new Error("A materialized ProjectDraft cannot be projected as active.");
  return value;
}
