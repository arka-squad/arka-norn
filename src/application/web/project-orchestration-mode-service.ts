...
import type { OrchestrationTrackingView, ProjectOrchestrationModeView } from "./contracts.js";
import { assertExpectedTimestamp, WebMutationError } from "./web-mutation-concurrency.js";

export interface ProjectOrchestrationModeDeps {
...
export async function projectOrchestrationModeView(
  configurations: OrchestrationConfigurationStore,
  project: Project,
  orchestrations: readonly OrchestrationTrackingView[],
): Promise<ProjectOrchestrationModeView> {
...
export async function setProjectOrchestrationMode(
  deps: ProjectOrchestrationModeDeps,
  projectId: string,
  input: { readonly mode: "manual" | "automatic"; readonly expectedUpdatedAt: string },
): Promise<void> {
  if (input.mode !== "manual" && input.mode !== "automatic") throw new WebMutationError(400, "invalid_orchestration_mode");
  const indexed = (await deps.management.projects.list()).find((candidate) => candidate.id.value === projectId);
  if (indexed === undefined) {
    if (await deps.framing.showProjectDraft(projectId) !== undefined) throw new WebMutationError(409, "project_draft_not_materialized");
  ...
  if (project.orchestrationMode === input.mode) return;
  if (input.mode === "automatic") {
    ...
    if (!configuration.automaticEnabled) await deps.configurations.save(project, configuration.activate(deps.now()));
  ...
  if (configuration?.automaticEnabled === true) await deps.configurations.save(project, configuration.deactivate(deps.now()));
}

async function requiredConfiguration(store: OrchestrationConfigurationStore, project: Project): Promise<OrchestrationConfiguration | undefined> {
  try {
