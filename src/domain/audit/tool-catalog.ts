...
import type { AuditModuleId } from "./audit-types.js";

export const AUDIT_TOOL_IDS = ["syft", "grype", "trivy", "gitleaks", "k6", "terraform", "node", "python", "go", "rust", "maven", "gradle"] as const;
export type AuditToolId = typeof AUDIT_TOOL_IDS[number];
...
export const AUDIT_TOOL_CATALOG: readonly AuditToolDefinition[] = Object.freeze([
...
export function auditToolDefinition(id: AuditToolId): AuditToolDefinition {
  const definition = AUDIT_TOOL_CATALOG.find((candidate) => candidate.id === id);
  if (definition === undefined) throw new Error(`Unknown audit tool: ${id}`);
  return definition;
}

export function isAuditToolId(value: unknown): value is AuditToolId {
  return typeof value === "string" && (AUDIT_TOOL_IDS as readonly string[]).includes(value);
}

function tool(id: AuditToolId, modules: readonly AuditModuleId[], image: string, network: AuditToolDefinition["network"]): AuditToolDefinition {
