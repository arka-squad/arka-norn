...
function selectedForAny(types: readonly string[], states: ReadonlyMap<string, StepState>, documents: readonly EvaluatedDocument[]): EvaluatedDocument | undefined {
...
function validateFeatureAndAuthors(input: PipelineEvaluationInput, documents: readonly EvaluatedDocument[], errors: string[]): void {
  ...
  for (const document of documents) {
    ...
    if (registry === undefined || (version !== 3 && version !== 5)) continue;
    ...
    const migratedFromV2 = version === 5
      ...
      && (migration as Readonly<Record<string, unknown>>)["source_schema_version"] === 2;
    const authorId = stringField(document.content, "author_agent_id");
    if (migratedFromV2 && authorId === undefined) continue;
...
function completionFor(input: {
  ...
  if (input.businessStatus === "passed") return "completed";
  if (input.businessStatus === "failed") return "failed";
  if (input.businessStatus === "in_progress") return "in_progress";
  return "blocked";
}
...
function overallStatusFor(steps: readonly StepState[], errors: readonly string[]): PipelineOverallStatus {
  if (errors.length > 0 || steps.some((step) => step.schemaStatus === "invalid")) return "invalid";
  if (steps.some((step) => step.completionStatus === "failed")) return "failed";
  if (steps.filter((step) => step.required).every((step) => step.completionStatus === "completed")) return "completed";
  return "incomplete";
}

function withoutContent(document: EvaluatedDocument) {
  return {
