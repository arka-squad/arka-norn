...
export const CURRENT_MARKER_SCHEMA_VERSION = 2 as const;
...
export function planProjectMarkerMigration(value: unknown): MarkerMigrationPlan<ProjectMarkerV2> {
...
): MarkerMigrationPlan<FeatureMarkerV2> {
...
export function isProjectMarkerV2(value: unknown): value is ProjectMarkerV2 {
  if (!isRecord(value) || value.schemaVersion !== CURRENT_MARKER_SCHEMA_VERSION) return false;
