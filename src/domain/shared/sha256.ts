...
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < byteLength; offset += 64) compress(view, offset, words, hash);
  return [...hash].map((word) => word.toString(16).padStart(8, "0")).join("");
}

function compress(view: DataView, offset: number, words: Uint32Array, hash: Uint32Array): void {
  ...
  for (let index = 0; index < 64; index += 1) {
    ...
    const temporary1 = (h + sum1 + choose + (ROUND_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
...
function rotate(value: number | undefined, bits: number): number {
  const word = value ?? 0;
