...
export class FramingService {
    ...
    async show(projectId, framingId) {
        ...
        if (plan === undefined)
            throw new Error("The selected framing journal cannot be reconstructed.");
        return plan;
    }
    showRevision(projectId, framingId, revision) {
        return this.dependencies.store.loadRevision(projectId, framingId, revision);
    }
    project(plan, view) {
        if (view === "summary")
            return summaryProjection(plan);
        if (view === "evidence")
            return evidenceProjection(plan);
        if (view === "map")
            return mapProjection(plan);
        return planProjection(plan);
    }
    async resume(projectId, framingId) {
    ...
    async applyDelta(projectId, framingId, delta) {
        for (let attempt = 0; attempt < 5; attempt += 1) {
            ...
            if (delta.baseRevision !== current.revision) {
                ...
                if (base === undefined)
                    throw new Error(`Framing base revision ${delta.baseRevision} is unavailable.`);
            ...
            try {
                ...
                if (!(error instanceof Error) || !error.message.startsWith("Framing revision conflict:"))
                    throw error;
            }
        }
    ...
    async publish(projectId, framingId) {
        const current = await this.show(projectId, framingId);
        ...
        if (context.draft !== null)
            project = await this.dependencies.projects.importFrom({ root: context.draft.root });
    ...
    async materializeDirectFeature(project, plan, published) {
        if (plan.target.kind !== "feature" || plan.target.origin !== "new")
            return;
        const pipelineId = plan.derivedState.recommendedPipelineId;
        if (pipelineId === null)
            throw new Error("A new Feature cannot be materialized without a calculated 2.3 delivery route.");
    ...
    async resolveTarget(project, existingFeatureId, newFeatureTitle) {
        if (existingFeatureId !== undefined) {
            ...
            if (!feature.projectId.equals(project.id))
                throw new Error(`Feature ${existingFeatureId} does not belong to Project ${project.id.value}.`);
            return {
        ...
        if (newFeatureTitle !== undefined) {
            ...
            if (title.length === 0)
                throw new Error("A new Feature requires a working title.");
            return {
    ...
    async findMatchingPlan(projectId, target) {
        const plan = await this.dependencies.store.load(projectId, target.framingId);
        if (plan === undefined || plan.publication !== null)
            return undefined;
        return plan;
    }
...
async function canonicalDirectory(value) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink())
        throw new Error(`Framing entry path must be a real directory: ${value}.`);
    return fs.realpath(root);
}
async function selectContainingProject(projects, path) {
    const candidates = [];
...
function derivedIdentifier(name, salt) {
...
function summaryProjection(plan) {
    return {
...
function evidenceProjection(plan) {
    return {
...
function changedKeys(base, current) {
    const keys = [];
    const sections = Object.keys(base.knowledge);
    for (const section of sections) {
        ...
        for (const id of ids) {
            ...
            if (JSON.stringify(before) !== JSON.stringify(after))
                keys.push(`knowledge:${section}:${id}`);
        }
    }
    if (JSON.stringify(base.repositoryProbe) !== JSON.stringify(current.repositoryProbe))
        keys.push("repositoryProbe");
    if (JSON.stringify(base.decomposition) !== JSON.stringify(current.decomposition))
        keys.push("decomposition");
    return keys;
}
function deltaKeys(delta) {
    return delta.operations.map((operation) => {
        ...
        if (operation.op === "invalidate_evidence")
            return `knowledge:evidence.claims:${operation.id}`;
...
function preserveConcurrentContradictions(delta, revision, overlaps) {
    ...
    for (const operation of delta.operations) {
        ...
        if (operation.op !== "upsert_knowledge")
            throw new Error(`Framing revision conflict on ${key}; preserve and reconfront the contradiction.`);
