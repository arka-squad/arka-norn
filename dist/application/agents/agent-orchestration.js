...
export function createAgentAdvice(state) {
    ...
    if (state.feature === undefined)
        warnings.push("No unique Feature is selected; Product must choose or create the priority before starting a specialized profile.");
...
export function createInitializationPrompt(state, input) {
    ...
    if (input.role !== "product" && feature === undefined)
        throw new InvalidAgentOptionError("feature", `role ${input.role} requires an explicit Feature`);
    const next = state.report?.nextActions[0];
    const requiredRole = next === undefined ? undefined : roleForAction(next);
...
export function createProductHandoffPrompt(state, requestedAgentId) {
    ...
    if (agent === undefined)
        throw new InvalidAgentOptionError("product", "no active main Product Agent is available for handoff");
    if (!agent.active || roleCategory(agent.role) !== "product")
        throw new InvalidAgentOptionError("product", `Agent ${agent.id.value} is not an active main Product Agent`);
...
function resolveProductPrincipal(state) {
    ...
    if (products.length === 1)
        return { sessionId: "main", status: "unbound", agentId: products[0].id.value, reason: `Product Agent ${products[0].id.value} must be bound to the main session.` };
    if (products.length === 0)
        return { sessionId: "main", status: "missing", reason: "No active Product Agent is registered; the first Project Agent must take this role." };
...
function productNextAction(state, requiredRole, stepId) {
    ...
    if (state.feature === undefined)
        return "Choose, create or import the priority Feature before starting a specialized profile.";
    if (state.report?.overallStatus === "completed")
        return "Verify handoffs, close the Feature and choose the next product priority.";
...
function recommendationsFor(requiredRole, stepId, feature, orchestrationMode) {
    if (requiredRole === "product")
        return [];
    if (orchestrationMode === "automatic")
        return [recommendation(requiredRole, "execute", stepId, feature, orchestrationMode)];
    const recommendations = [recommendation(requiredRole, "execute", stepId, feature, orchestrationMode)];
    if (["audit", "architecte"].includes(requiredRole))
        recommendations.push(recommendation("dev", "prepare", stepId, feature, orchestrationMode));
    if (requiredRole === "dev")
        recommendations.push(recommendation("qa", "prepare", stepId, feature, orchestrationMode));
    return recommendations;
}
function recommendation(role, mode, stepId, feature, orchestrationMode) {
...
function renderInitializationPrompt(state, provider, policy, sessionId, mode, expectedStepId, existingAgent, preflightCommand) {
    const feature = state.feature;
...
function canReuseBinding(agent, role, project, feature) {
    if (!agent.active || roleCategory(agent.role) !== role || !agent.scope.projectId.equals(project.id))
        return false;
    if (feature === undefined || agent.scope.featureIds.length === 0)
        return true;
    return agent.scope.featureIds.some((id) => id.equals(feature.id));
}
function resolveSessionAgent(state, sessionId, role, feature, provider) {
    ...
    if (provider?.trim() === "")
        throw new InvalidAgentOptionError("provider", "provider cannot be empty");
    if (provider === undefined)
        throw new InvalidAgentOptionError("provider", `--provider is required to create the ${role} identity for session ${sessionId.value}`);
    return undefined;
}
function relativeFeaturePath(projectRoot, featureRoot) {
