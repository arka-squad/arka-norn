...
function resolveSessionAgent(state, sessionId, role, feature, provider) {
    ...
    if (provider?.trim() === "")
        throw new InvalidAgentOptionError("provider", "le provider ne peut pas être vide");
    if (provider === undefined)
        throw new InvalidAgentOptionError("provider", `--provider est requis pour créer l'identité ${role} de la session ${sessionId.value}`);
    return undefined;
}
