...
import { isAssessmentStatus, isAuditDepth, isAuditMode, isAuditModuleId, isConfidenceLevel, isFindingOrigin, isFindingSeverity, isModuleExecutionStatus, } from "../../domain/audit/audit-types.js";
...
export function parseAuditRequest(value) {
    if (!isRecord(value))
    ...
    if (!Array.isArray(value["modules"]) || value["modules"].length === 0)
        throw new Error("Audit request modules must not be empty");
    const modules = value["modules"].map((candidate, index) => {
        ...
        if (candidate["intent"] !== "discover" && candidate["intent"] !== "audit")
            throw new Error(`Audit module ${candidate["moduleId"]} intent is invalid`);
        if (!isAuditDepth(candidate["depth"]))
            throw new Error(`Audit module ${candidate["moduleId"]} depth is invalid`);
        const intent = candidate["intent"];
        if (depthRank(candidate["depth"]) > depthRank(auditModuleDefinition(candidate["moduleId"]).maximumDepth))
            throw new Error(`Audit module ${candidate["moduleId"]} does not support depth ${candidate["depth"]}`);
        return {
    ...
    if (!credentialRefs.every((item) => /^[A-Z][A-Z0-9_]{0,63}$/.test(item)))
        throw new Error("Credential references must be environment variable names");
    ...
    if (typeof capabilities["allowImagePulls"] !== "boolean")
        throw new Error("capabilities.allowImagePulls must be boolean");
    const referencedHosts = [...urls, ...dynamicTargets].map((url) => new URL(url).hostname.toLowerCase());
    if (referencedHosts.some((host) => !allowedHosts.includes(host)))
        throw new Error("Every source or target URL host must be explicitly allowed");
    const request = {
        objective,
        mode: value["mode"],
        paths: unique(paths),
...
export function parseModuleResult(value, expectedAuditId, expectedModuleId) {
    ...
    if ((value["intent"] !== "discover" && value["intent"] !== "audit") || !isAuditDepth(value["depth"]))
        throw new Error("Audit module result intent or depth is invalid");
    const execution = record(value["execution"], "execution");
    if (!isModuleExecutionStatus(execution["status"]))
        throw new Error("Audit module execution status is invalid");
    ...
    if (assessmentValue !== null) {
        ...
        if (!isAssessmentStatus(entry["status"]) || !isConfidenceLevel(entry["confidence"]))
            throw new Error("Audit module assessment is invalid");
        assessment = { status: entry["status"], confidence: entry["confidence"] };
    }
    if (value["intent"] === "discover" && assessment !== null)
        throw new Error("Discovery module assessment must be null");
    if (value["intent"] === "audit" && assessment === null)
        throw new Error("Audit module assessment is required");
    ...
    if (assessment?.status === "pass" && (execution["status"] !== "complete" || coverage.missing.length > 0))
        throw new Error("A pass assessment requires complete execution and coverage");
    ...
    if (assessment?.status === "pass" && findings.some((finding) => finding.severity === "critical"))
        throw new Error("A critical finding forbids a pass assessment");
    const result = {
        schemaVersion: 1,
...
export function assertNoSecretMaterial(value) {
    if (SECRET_PATTERN.test(JSON.stringify(value)))
        throw new Error("Audit payload contains credential-like material");
}
function parseEvidence(value, index) {
    const entry = record(value, `evidence.${index}`);
    if (entry["kind"] !== "command" && entry["kind"] !== "file" && entry["kind"] !== "metric" && entry["kind"] !== "external")
        throw new Error("Audit evidence kind is invalid");
    if (entry["classification"] !== "public" && entry["classification"] !== "internal" && entry["classification"] !== "sensitive")
        throw new Error("Audit evidence classification is invalid");
    if (typeof entry["redacted"] !== "boolean")
        throw new Error("Audit evidence redacted must be boolean");
    return {
...
function parseFinding(value, index, evidenceIds) {
    const entry = record(value, `findings.${index}`);
    if (!isFindingSeverity(entry["severity"]) || !isConfidenceLevel(entry["confidence"]) || !isFindingOrigin(entry["origin"]))
        throw new Error("Audit finding classification is invalid");
    if (entry["status"] !== "open" && entry["status"] !== "accepted" && entry["status"] !== "resolved")
        throw new Error("Audit finding status is invalid");
    const finding = {
    ...
    if (finding.evidenceIds.some((id) => !evidenceIds.has(id)))
        throw new Error(`Finding ${finding.id} references unknown evidence`);
    if (finding.origin === "inferred" && finding.evidenceIds.length === 0)
        throw new Error(`Inferred finding ${finding.id} requires evidence`);
    if (finding.fingerprint !== findingFingerprint(finding))
        throw new Error(`Finding ${finding.id} fingerprint is invalid`);
    return finding;
}
function normalizeRelativePath(value) {
    const normalized = value.replaceAll("\\", "/").replace(/^\.\//, "").replace(/\/+$/, "") || ".";
    if (normalized.startsWith("/") || /^[A-Za-z]:\//.test(normalized) || normalized.split("/").includes(".."))
        throw new Error(`Path is outside the Project: ${value}`);
    return normalized;
}
function validateHttpUrl(value) {
    ...
    if (url.search !== "" || url.hash !== "")
        throw new Error("Audit URLs must not contain query parameters or fragments");
    return url.toString();
}
...
function validateDynamicTargetUrl(value) {
    const normalized = validateHttpUrl(value);
    const host = new URL(normalized).hostname.toLowerCase();
    if (host === "metadata.google.internal" || (isPrivateIpv4Literal(host) && !host.startsWith("127.")))
        throw new Error("Dynamic target must be public or explicit localhost");
    return normalized;
}
...
function printable(value, field, maximum) {
...
function stringArray(value, field, nonEmpty) {
