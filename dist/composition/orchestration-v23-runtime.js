    ...
    return {
        async preview(input) {
            ...
            if (!feature.belongsTo(project.id))
                throw new Error("Feature does not belong to the selected Project.");
            const configuration = await deps.configurations.load(project);
            if (project.orchestrationMode !== "automatic" || configuration?.automaticEnabled !== true)
                throw new Error("Norn 2.3 automatic orchestration is not activated for this Project.");
            const campaignId = newCampaignId(feature.id.value, now());
            const issues = [];
            let assigned;
            try {
                assigned = await loadTaskPlans(feature, project, await deps.agents.list(project));
            }
            catch (error) {
                issues.push({ code: classifyPlanningError(error), message: safeMessage(error) });
            }
            ...
            if (enabled.length === 0)
                issues.push({ code: "gateway_profile_missing", message: "No enabled execution profile is configured." });
            const eligible = plan !== undefined && preflights.some((preflight) => preflight.healthy);
            if (eligible)
                await deps.campaigns.savePlan(plan);
        ...
        async start(input) {
            ...
            if (selected.length !== selectedIds.length || selected.some((profile) => !profile.enabled))
                throw new Error("Run authorization selects a missing or disabled execution profile.");
            for (const task of plan.tasks)
                assertProfileRequirement(task.role, configuredProfiles.get(input.profileByRole[task.role] ?? ""), task.requiredProfile);
            ...
            if (unhealthy.length > 0)
                throw new Error(`Authorized execution profile preflight failed: ${unhealthy.map((value) => `${value.profileId}/${value.code}`).join(", ")}`);
            const prepared = new Map();
            for (const profile of selected)
                prepared.set(profile.id, await deps.profiles.prepare(profile));
        ...
        async apply(input) {
            ...
            if (plan === undefined || artifact === undefined)
                throw new Error("Campaign application candidate was not found.");
            if (artifact.fingerprint !== input.confirmationFingerprint)
                throw new Error("Campaign application fingerprint does not match the candidate.");
            if (artifact.risk.hardDenials.length > 0)
                throw new Error("A candidate with a global risk denial cannot be applied.");
            if (await deps.campaigns.loadApplication(project.id.value, input.campaignId) !== undefined)
                throw new Error("Campaign candidate is already applied.");
            const appliedCommit = await deps.git.applyFastForward(project, plan.props.snapshot, artifact.integration);
            const applicationCandidate = { schemaVersion: 1, candidateFingerprint: artifact.fingerprint, appliedCommit, recordedAt: now() };
            const application = Object.freeze({ ...applicationCandidate, fingerprint: resultFingerprint(applicationCandidate) });
...
async function runCampaign(input) {
    ...
    while (pending.size > 0) {
        for (const task of input.plan.tasks) {
            if (!pending.has(task.id) || !task.dependencies.some((dependency) => terminalFailures.has(dependency)))
                continue;
        ...
        for (const result of results) {
            ...
            if (result.commit === undefined)
                terminalFailures.add(result.taskId);
            else {
                completed.add(result.taskId);
                commits.set(result.taskId, result.commit);
            }
        }
    }
    if (pending.size > 0)
        for (const taskId of pending) {
            terminalFailures.add(taskId);
            await input.emit("task_blocked", taskId, "dag_unreachable");
        }
    ...
    if (integration.status === "conflicted")
        integration = await resolveIntegration(input, integration, budget);
    if (integration.status === "conflicted") {
        await input.emit("campaign_blocked", undefined, "integration_conflict");
        return currentResult(input.project.id.value, input.plan.props.id, input.deps.events);
    }
    ...
    if (applicationGate === undefined) {
        try {
            appliedCommit = await input.deps.git.applyFastForward(input.project, input.plan.props.snapshot, integration);
        }
        catch (error) {
            applicationGate = { code: "baseline_diverged", message: safeMessage(error) };
        }
    }
    const candidate = { schemaVersion: 1, integration, commits: orderedCommits, risk, ...(appliedCommit === undefined ? {} : { appliedCommit }), ...(applicationGate === undefined ? {} : { applicationGate }), recordedAt: input.now() };
    const artifact = Object.freeze({ ...candidate, fingerprint: resultFingerprint(candidate) });
...
async function allocateReadyWorkspaces(tasks, input) {
...
async function resolveIntegration(input, conflict, budget) {
    const task = { id: "integration", agentId: input.plan.props.integrationAgentId, role: input.plan.props.integrationRole, requiredProfile: input.plan.props.integrationRequiredProfile, priority: 10_000, dependencies: input.plan.tasks.map((entry) => entry.id), readScopes: ["."], writeScopes: conflict.conflictPaths.length === 0 ? ["."] : [...conflict.conflictPaths], deliverables: ["Resolve the recorded integration conflict without dropping validated intent."], validations: ["Run the declared test recipe after resolving the conflict."] };
    ...
    if (resolved.status === "conflicted")
        resolved = await input.deps.git.buildPriorityFallback(input.project, resolved);
...
async function failIntegrationAttempt(input, task, profileId, executionId, conflict, startedAt, code, proofs) {
...
async function executeTask(task, input, budget, workspaceAllocation, hardStop) {
    ...
    if (admission.action === "block_new" || admission.action === "stop") {
        await input.emit("task_budget_stopped", task.id, "budget_exceeded");
        return { taskId: task.id };
    }
    const profile = input.configuredProfiles.get(profileId);
    const runtime = input.prepared.get(profileId);
    if (profile === undefined || runtime === undefined) {
        await input.emit("task_blocked", task.id, "profile_missing");
        return { taskId: task.id };
    }
    const executionId = `execution-${task.id}-1`;
    const startedAt = input.now();
    let workspace;
    const attemptJournal = { state: "none" };
    try {
        ...
        if (!validProofs(outcome.proofReferences)) {
            await terminalAttempt("failed", "proof_missing", outcome.proofReferences);
            await input.emit("task_failed", task.id, "proof_missing");
            return { taskId: task.id };
        }
        if (!input.authorization.props.allowCommits) {
            await terminalAttempt("blocked", "commit_not_authorized", outcome.proofReferences);
            await input.emit("task_blocked", task.id, "commit_not_authorized");
            return { taskId: task.id };
        }
        ...
        async function terminalAttempt(status, code, proofs) {
...
function eventWriter(store, projectId, campaignId, now, initialRevision = 0) {
    let revision = initialRevision;
    let queue = Promise.resolve();
    return async (kind, taskId, codeOrFingerprint) => {
        queue = queue.then(async () => {
            ...
            const event = { schemaVersion: 1, campaignId, revision, kind, ...(taskId === undefined ? {} : { taskId }), ...(code === undefined ? {} : { code }), fingerprint, at: now() };
            await store.append(projectId, event);
        });
        await queue;
    };
}
async function currentResult(projectId, campaignId, events, artifact) { const projection = projectCampaignEvents(await events.load(projectId, campaignId)); if (projection === undefined)
    throw new Error("Campaign event projection is unavailable."); return Object.freeze({ campaignId, projection, ...(artifact === undefined ? {} : { artifact }) }); }
function unionScopes(tasks) { const scopes = [...new Set(tasks.flatMap((task) => [...task.writeScopes]))]; return scopes.includes(".") ? ["."] : scopes.sort(); }
function policyFingerprint(value) { return digest(JSON.stringify(value)); }
function authorizationFingerprint(value) { return digest(JSON.stringify(value, dateReplacer)); }
function resultFingerprint(value) { return digest(JSON.stringify(value, dateReplacer)); }
function newCampaignId(featureId, at) { const feature = featureId.toLocaleLowerCase("en").replace(/[^a-z0-9._-]+/gu, "-").slice(0, 72); return `campaign-${feature}-${at.getTime().toString(36)}`; }
function classifyWorkspaceError(error) { const message = safeMessage(error); return /limit|too large|exceed/iu.test(message) ? "workspace_limit" : "workspace_unavailable"; }
...
function determineApplicationGate(input, riskEligible, priorityFallback) {
    ...
    if (!input.plan.props.snapshot.clean)
        return { code: "dirty_snapshot", message: "The campaign snapshot contains authorized local changes and cannot be applied automatically." };
    if (!riskEligible)
        return { code: "risk_gate", message: "The candidate exceeds the authorized automatic-application risk policy." };
    if (priorityFallback)
        return { code: "priority_fallback", message: "The priority fallback discarded conflict hunks and requires human validation." };
    return undefined;
}
function safeMessage(error) { return (error instanceof Error ? error.message : String(error)).replace(/[\u0000-\u001f\u007f]+/gu, " ").slice(0, 500); }
function normalizeCode(value) { const normalized = value.toLocaleLowerCase("en").replace(/[^a-z0-9._-]+/gu, "_").replace(/^_+|_+$/gu, "").slice(0, 100); return /^[a-z0-9]/u.test(normalized) ? normalized : "orchestration_error"; }
function validProofs(values) { return values.some((value) => /^receipt-recipe-(?:test|build|typecheck|lint)-pass-/u.test(value)) && values.every((value) => /^receipt-[A-Za-z0-9-]+$/u.test(value)); }
function digest(value) { return createHash("sha256").update(value).digest("hex"); }
function executionProfileFingerprint(profile) { return digest(JSON.stringify(profile.props, dateReplacer)); }
function assertProfileRequirement(role, profile, requirement) { if (profile === undefined || !requirement.transports.includes(profile.transport) || requirement.capabilities.some((capability) => !profile.props.capabilities.includes(capability)))
    throw new Error(`Execution profile for role ${role} does not satisfy the signed TaskPlan requirement.`); }
function dateReplacer(_key, value) { return value instanceof Date ? value.toISOString() : value; }
//# sourceMappingURL=orchestration-v23-runtime.js.map