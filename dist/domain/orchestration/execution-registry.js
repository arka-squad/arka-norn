...
export class ExecutionRegistry {
    ...
    static empty(projectId, updatedAt = new Date(0)) {
    ...
    add(record, updatedAt) {
        ...
        if (this.find(record.id) !== undefined)
            throw new InvalidExecutionRegistryError(`execution ${record.id} already exists`);
        return this.withExecutions([...this.executions, record], updatedAt);
    }
    replace(record, updatedAt) {
        this.assertProject(record);
    ...
    withExecutions(executions, updatedAt) {
...
function validateRegistryProps(props) {
    ...
    if (!isUnknownArray(executions))
        throw new InvalidExecutionRegistryError("executions must be an array");
    const ids = new Set();
    for (const record of executions) {
        if (!(record instanceof ExecutionRecord))
            throw new InvalidExecutionRegistryError("executions must contain ExecutionRecord values");
        if (!record.order.scope.projectId.equals(props.projectId))
            throw new InvalidExecutionRegistryError(`execution ${record.id} belongs to another project`);
        if (ids.has(record.id))
            throw new InvalidExecutionRegistryError(`duplicate execution ${record.id}`);
        ids.add(record.id);
    }
...
function validateDate(value, field) {
    if (!(value instanceof Date) || Number.isNaN(value.getTime()))
        throw new InvalidExecutionRegistryError(`${field} must be a valid date`);
}
function isUnknownArray(value) {
    return Array.isArray(value);
}
//# sourceMappingURL=execution-registry.js.map