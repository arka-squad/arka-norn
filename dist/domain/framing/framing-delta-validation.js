...
export function assertPlanDeltaContract(value) {
    const delta = object(value, "Framing delta");
    exact(delta, ["schemaVersion", "planId", "baseRevision", "operations", "reason"], "Framing delta");
    if (delta.schemaVersion !== 1 || !id(delta.planId) || !integer(delta.baseRevision, 1) || !text(delta.reason, 1, 512))
        throw new Error("Invalid framing delta envelope.");
    if (!Array.isArray(delta.operations) || delta.operations.length < 1 || delta.operations.length > 128)
        throw new Error("Framing delta requires 1..128 operations.");
...
function assertOperation(value) {
    ...
    if (operation.op === "upsert_knowledge") {
        ...
        if (!SECTIONS.has(String(operation.section)))
            throw new Error("Invalid framing section.");
    ...
    if (operation.op === "supersede_knowledge") {
        ...
        if (!SECTIONS.has(String(operation.section)) || !id(operation.id) || !id(operation.supersededBy))
    ...
    if (operation.op === "invalidate_evidence") {
        ...
        if (!id(operation.id) || !id(operation.supersededBy))
            throw new Error("Invalid evidence invalidation.");
        return;
    }
...
function assertKnowledgeInput(value) {
    ...
    if (!id(input.id) || !text(input.statement, 1, 4_000))
        throw new Error("Invalid framing knowledge input.");
    ...
    if (!PROVENANCE.has(String(provenance.kind)) || !text(provenance.reference, 1, 512))
        throw new Error("Invalid knowledge provenance.");
    if (input.blocksProgress !== undefined && typeof input.blocksProgress !== "boolean")
        throw new Error("Invalid knowledge blocking flag.");
    if (input.dependsOn !== undefined)
        assertIdArray(input.dependsOn, "Knowledge dependencies");
}
function assertProbe(value) {
    ...
    if (!(snapshot.gitCommit === null || (typeof snapshot.gitCommit === "string" && /^[a-f0-9]{40,64}$/u.test(snapshot.gitCommit))) || !hash(snapshot.workspaceFingerprint))
    ...
    for (const field of ["files", "sourceFiles", "testFiles", "manifestFiles", "constraintFiles", "symlinks", "submodules"])
        if (!integer(inventory[field], 0))
            throw new Error("Invalid repository inventory count.");
    if (typeof inventory.truncated !== "boolean")
        throw new Error("Invalid repository inventory truncation flag.");
    assertStringArray(inventory.ignoredRoots, "Ignored roots", false);
    if (!Array.isArray(probe.reasons))
        throw new Error("Invalid repository probe reasons.");
    for (const reasonValue of probe.reasons) {
        ...
        if (typeof reason.code !== "string" || !/^[a-z0-9_]+$/u.test(reason.code) || !text(reason.evidenceRef, 1, 512))
            throw new Error("Invalid repository probe reason.");
    }
}
function assertDecomposition(value) {
    ...
    if (decomposition.kind === "project_features") {
        ...
        if (!Array.isArray(decomposition.features))
            throw new Error("Project decomposition requires Features.");
        for (const valueEntry of decomposition.features) {
            ...
            if (!id(entry.candidateId) || !text(entry.title) || !text(entry.observableOutcome) || !text(entry.acceptanceScenario) || !text(entry.cohesionRationale))
                throw new Error("Invalid Feature candidate.");
    ...
    if (decomposition.kind !== "feature_lots")
        throw new Error("Invalid framing decomposition kind.");
    exact(decomposition, ["kind", "lots"], "Feature decomposition");
    if (!Array.isArray(decomposition.lots))
        throw new Error("Feature decomposition requires Lots.");
    for (const valueEntry of decomposition.lots) {
        ...
        if (!id(entry.id) || !text(entry.title) || !text(entry.objective) || !text(entry.observableEffect))
            throw new Error("Invalid Feature Lot.");
        ...
        for (const kind of ["functional", "ux", "code", "security"])
            assertStringArray(proofs[kind], `Lot ${kind} proofs`, false);
    }
}
function object(value, label) { if (typeof value !== "object" || value === null || Array.isArray(value))
    throw new Error(`${label} must be an object.`); return value; }
function exact(value, keys, label) { exactOptional(value, keys, [], label); }
function exactOptional(value, required, optional, label) { const keys = Object.keys(value); if (required.some((key) => !(key in value)) || keys.some((key) => !required.includes(key) && !optional.includes(key)))
    throw new Error(`${label} has missing or unknown properties.`); }
function id(value) { return typeof value === "string" && /^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$/u.test(value); }
function text(value, minimum = 1, maximum = 4_000) { return typeof value === "string" && value.trim().length >= minimum && value.length <= maximum; }
function integer(value, minimum) { return typeof value === "number" && Number.isInteger(value) && value >= minimum; }
function hash(value) { return typeof value === "string" && /^[a-f0-9]{64}$/u.test(value); }
function date(value) { return typeof value === "string" && !Number.isNaN(Date.parse(value)); }
function assertIdArray(value, label) { if (!Array.isArray(value) || value.some((item) => !id(item)) || new Set(value).size !== value.length)
    throw new Error(`${label} must contain unique identifiers.`); }
function assertStringArray(value, label, nonEmpty) { if (!Array.isArray(value) || (nonEmpty && value.length === 0) || value.some((item) => !text(item)))
    throw new Error(`${label} must contain strings.`); }
//# sourceMappingURL=framing-delta-validation.js.map