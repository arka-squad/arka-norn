...
export async function migrateLegacyFeatureContract(input) {
    ...
    if (!isFeatureMarkerV3(marker) && !isFeatureMarkerV4(marker))
        throw new Error(`Unsupported Feature marker for contract migration: ${markerPath}`);
    ...
    if (isFeatureMarkerV4(marker)) {
        if (planned.length > 0)
            throw new Error(`Feature ${marker.id} has a v4 marker but still contains legacy documents.`);
    ...
    for (const document of planned)
        await createBackup(document.backupPath, document.raw);
    const written = [];
    try {
        for (const document of planned) {
        ...
        for (const document of written.reverse())
            await writeFileAtomic(document.path, document.raw, { mode: 0o644 }).catch(() => undefined);
...
async function planDocuments(featureRoot, schemas, validator) {
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
        ...
        try {
            ...
            if (looksLikePipelineDocument(raw))
                throw error;
            continue;
        }
        const sourceType = content["type"];
        const sourceVersion = content["schema_version"];
        if (sourceType === undefined && sourceVersion === undefined)
            continue;
        ...
        if (sourceVersion === 5)
            continue;
        if (sourceVersion < 2 || sourceVersion > 4)
            throw new Error(`Unsupported schema_version ${sourceVersion} in ${path}`);
        const type = canonicalDocumentType(sourceType);
        const schemaPath = schemas.get(type);
        if (schemaPath === undefined)
            throw new Error(`Document type ${sourceType} is not part of the Feature pipeline: ${path}`);
        ...
        if (!validation.valid)
            throw new Error(`Migrated document would be invalid (${basename(path)}): ${validation.errors.join("; ")}`);
...
function assertIdentityAndRelationsPreserved(source, migrated, path) {
    for (const field of ["id", "feature_id", "sequence", "created_at", "author_agent_id", "depends_on_document_ids"]) {
        if (JSON.stringify(source[field]) !== JSON.stringify(migrated[field]))
            throw new Error(`Migration changed preserved field ${field}: ${path}`);
    }
}
function deterministicMigrationDate(content) {
...
function documentResult(document) {
    return { path: document.path, type: document.type, fromSchemaVersion: document.sourceVersion, toSchemaVersion: 5 };
}
async function canonicalDirectory(path) {
    const stat = await fs.lstat(path);
    if (!stat.isDirectory() || stat.isSymbolicLink())
        throw new Error(`Feature migration target must be a regular directory: ${path}`);
    return fs.realpath(resolve(path));
}
async function requiredRaw(path) {
    const raw = await readRaw(path);
    if (raw === undefined)
        throw new Error(`File not found: ${path}`);
    return raw;
}
function parseRecord(raw, path) {
    const parsed = JSON.parse(raw);
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed))
        throw new Error(`Expected a JSON object: ${path}`);
    return parsed;
}
function looksLikePipelineDocument(raw) {
    return raw.includes("schema_version") || raw.includes('"type"');
}
async function createBackup(backupPath, raw) {
    try {
        ...
        if (existing !== raw)
            throw new Error(`Existing migration backup differs from source: ${backupPath}`);
    }
}
//# sourceMappingURL=legacy-feature-migrator.js.map