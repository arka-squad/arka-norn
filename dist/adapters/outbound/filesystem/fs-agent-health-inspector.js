...
export class FsAgentHealthInspector {
    ...
    async inspectSession() {
        ...
        if (raw instanceof Error)
            return check("agents.session", "fail", raw.message);
        if (raw === undefined)
            return check("agents.session", "pass", "no local agent selection yet");
        try {
    ...
    async inspectRegistries() {
        const raw = await readRaw(join(this.home, ".arka-norn", "index", "projects.json")).catch(() => undefined);
        if (raw === undefined)
            return check("agents.registries", "warn", "project index absent; no agent registry to verify");
        let value;
        try {
        ...
        const inspections = await Promise.all(value.entries.map(async (entry) => {
            ...
            try {
                ...
                return { id: entry.id, status: "invalid" };
            }
        }));
        const invalid = inspections.filter((item) => item.status === "invalid");
        if (invalid.length > 0)
            return check("agents.registries", "fail", `${invalid.length}/${inspections.length} invalid agent registry: ${invalid.map((item) => item.id).slice(0, 3).join(", ")}`);
        const missing = inspections.filter((item) => item.status === "missing");
        if (missing.length > 0)
            return check("agents.registries", "warn", `${missing.length}/${inspections.length} project(s) without an agent registry; register an identity before producing`);
    ...
    async inspectProjectContext() {
        if (this.target === undefined)
            return check("project.context", "pass", "no target Project context requested");
        ...
        if (!hasMarker && registry === undefined)
            return check("project.context", "pass", "target directory is not an initialized Project");
        if (!hasMarker)
            return check("project.context", "fail", "agent registry exists without a Project marker in target directory");
        let project;
        try {
        ...
        if (registry === undefined)
            return check("project.context", "warn", `Project ${project.id.value} has no agent registry yet`);
        try {
    ...
    async invalidSelections(selections) {
...
//# sourceMappingURL=fs-agent-health-inspector.js.map