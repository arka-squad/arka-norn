...
async function runLocalizedCli(argv, homeDir, env, localeOverride) {
    const command = argv[0];
    const rest = argv.slice(1);
    const wantsJson = argv.includes("--json");
    const compatibilityWarnings = deprecatedAliasWarnings(argv);
    try {
        ...
        if (command === undefined || command === "config")
            return launchTui();
        if (command === "selftest")
...
function shouldRemind(command, rest) {
...
function normalizePublicJson(result, command, extraWarnings) {
    let parsed;
    try {
    ...
    if (envelope["schemaVersion"] === 2) {
        ...
        const aliasDiagnostics = extraWarnings.map((_warning, index) => ({ code: "deprecated_alias", params: { index } }));
        const normalized = {
            ...envelope,
...
async function runSelftest(argv) {
    ...
    const loaded = await import(pathToFileURL(resolve(FRAMEWORK_ROOT, "scripts", "selftest.mjs")).href);
    if (!isSelftestModule(loaded))
        throw new Error(translate("cli.error.invalidSelftest"));
    await loaded.runSelftest();
    return process.exitCode === undefined ? 0 : Number(process.exitCode);
}
function isSelftestModule(value) {
